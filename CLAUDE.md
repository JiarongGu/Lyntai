...
**Released: v3.2.0 (2026-09-19).** Twelve packages, one of them unreleased; public API frozen under SemVer 2.0 since 1.0, with no
carve-out (**D70**). The reasoning is `docs/DECISIONS.md`, **D1–D171** — read its generated index table
...
**The baseline a green run should match:** `3875 passed / 3913 total, 38 skipped` (the skips are
...
**MEASURED with Docker up, re-attested 2026-09-23 at `0ce6d8ea`** (+6 against `10bb3563`: the LLM
judge's content-option tests, a theory's two cases among them — archive Part 276; +2 before that, the stream
reader's two session-start tests, archive Part 275; and a Docker-down run the same day read `3663 / 242`,
