...
internal static class MemoryDensitySweep
{
    ...
    public static async Task<int> RunAsync(bool acrossLanguages = false)
    {
        ...
        async ValueTask RunOneAsync(CorpusLanguage language, Fixture fixture)
        {
            ...
            var engine = new GraphMemoryEngine("density", new SqliteMemoryGraphStore(db.Factory), seams: new GraphMemorySeams
                {
    ...
    private static readonly Fixture[] EnglishFixtures = BuildLanguageFixtures(
        ...
        [
            ...
            "the puppy finally learned to fetch the newspaper",
            "a violin string snapped during the second movement",
            "a flat tire delayed the delivery van by an hour",
            "the crossword's last clue stumped everyone at the table",
            "the spreadsheet formula kept returning the wrong total",
        ],
    ...
    private static readonly Fixture[] JapaneseFixtures = BuildLanguageFixtures(
        ...
        [
            ...
            "子犬はついに新聞をくわえてくることを覚えた",
            "第二楽章でバイオリンの弦が切れた",
            "配送トラックがパンクして一時間遅れた",
            "クロスワードの最後のヒントが全員を困らせた",
            "表計算の数式がいつも間違った合計を返した",
        ],
    ...
    private static readonly Fixture[] KoreanFixtures = BuildLanguageFixtures(
        ...
        [
            ...
            "강아지는 마침내 신문을 물어오는 법을 배웠다",
            "이악장 도중 바이올린 줄이 끊어졌다",
            "배송 트럭이 펑크가 나서 한 시간 늦어졌다",
            "크로스워드의 마지막 힌트가 모두를 골치 아프게 했다",
            "스프레드시트 수식이 계속 잘못된 합계를 냈다",
        ],
    ...
    private static readonly Fixture[] ChineseMixedFixtures = BuildLanguageFixtures(
        ...
        [
            ...
            "邻居的cat总是睡在porch的家具上",
            "puppy终于学会了叼newspaper",
            "violin的一根弦在第二movement断了",
            "delivery van爆胎耽误了一个小时",
            "crossword最后一题难倒了所有人",
            "spreadsheet公式总是算出错误的total",
        ],
