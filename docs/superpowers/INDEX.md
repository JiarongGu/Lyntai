...
|---|---|---|---|---|---|
| 2026-09-24 | Live routing moves a ROUTE, not half of one — design (the consumer's live override becomes a `provider:model[, …]` route; amended by the owner's ruling that the tool-loop probe follow it) and its two-task plan. It superseded the Part's first build, a per-provider model map never released | unreleased | ✓ | ✓ | **D176** (the rule, the rejected shapes, the probe's rejected snapshot, when each warning fires) · `docs/task-archive.md` Part 284 · `CHANGELOG.md` §Unreleased (the upgrade actions) |
| 2026-09-24 | A remember reports what the write did — design (the write side of `MemoryRecall.Ran`, chosen over an additive opt-in flag) and its two-task plan. Shipped `MemoryWriteResult` and the graph engine's vector-before-links order | unreleased | ✓ | ✓ | **D175** (the rule, the rejected alternatives, the known limits, the deferred readiness probe's trigger) · `docs/task-archive.md` Part 285 |
| 2026-09-23 | The file-system graph store — design (the journal layout, chosen on a measurement; one in-process core for both stores) and its seven-task plan. Shipped the memory engine's store as files | unreleased | ✓ | ✓ | **D174** (the layout, the shared core, the rejected alternatives) · `docs/task-archive.md` Part 282 · `docs/memory-measurements.md` §5 (`storage-writes-recall-writeback`) · `pitfalls.md` (a line kind picked by a property name misparses a DATA field) |
| 2026-09-23 | The next release: pre-release review of `v3.2.0..HEAD` and the regular wide check — a session plan | unreleased — executed 2026-09-23 | — | ✓ | `docs/task-archive.md` Part 279; the two rulings in **D161**; the file rule in **D171**; `docs/FIXES.md` 2026-09-23 (two entries); the dependency ruling, **D172** (`docs/task-archive.md` Part 280) |
...
| 2026-09-21 | `Lyntai.Storage.FileSystem` — roster-first implementation plan. Shipped five domains; the scan measurement overturned the plan's roster reasoning and its eager guard | unreleased | — | ✓ | **D171** (the shape, the roster, both REJECTED shapes) · `docs/task-archive.md` Part 277 · `docs/memory-measurements.md` §5 (`storage-scan-recall-cost`) · the graph store is `docs/task-archive.md` Part 282 |
...
| 2026-09-11 | ONE model serving MANY seams, priced against one per seam (`memory-contention`) | unreleased | ✓ | ✓ | `docs/task-archive.md` Part 190 · `docs/memory-measurements.md` §5 (`contention-mixed-recall-quiet-rerank`) · `pitfalls.md` (router mode is a SUPERVISOR; `--embedding`/`--reranking` are process-wide; a port check must test LISTENING; a bare `HttpClient` leaves proxy resolution on) |
| 2026-09-11 | What `VerdictCombination` costs a READER (`memory-locomo --verdict`) — the backlog predicted a null and the run refuted it | unreleased | ✓ | ✓ | `docs/task-archive.md` Part 191 · `docs/memory-measurements.md` §5 (two rows, partition and `+enginefuse`) · `docs/FIXES.md` (a bench printed a hardcoded caveat about its own output) |
...
| 2026-08-26 | The "superhuman memory" proposal — assessment, then Phase 1 and Phase 2 | unreleased | ✓ | ✓ | **D90** · design §5.7.0 · `docs/memory.md` §7 · `docs/FIXES.md` (two entries) · `pitfalls.md` (§Second doors, §Environment, CLI) · `windows-machine.md` · `docs/task-archive.md` Parts 97, 98, 100 · `TASKS.md` Part 99 |
| 2026-08-26 | Graph-engine COST at 1k / 10k / 100k (`memory-scale`, the §7 blind spot) — plus a `--repeat 5` run that settled the read-vs-write-back split the single-cell run could not | unreleased | — | 2 records | `docs/memory.md` §7 · `docs/task-archive.md` Part 97 |
