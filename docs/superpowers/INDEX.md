...
|---|---|---|---|---|---|
| 2026-09-26 | Per-request CLI tools — design (a request-aware provisioner seam taking a `CliToolRequest`, and a per-consumer tool map on the MCP host, chosen over a request field that would change meaning across a fallback) and its three-task plan | unreleased | ✓ | ✓ | **D190** (the rule and the three rejected shapes) |
