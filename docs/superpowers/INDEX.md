...
|---|---|---|---|---|---|
| 2026-09-17 | The provider seam: one interface per SIGNATURE over a generic routed base, so a consuming app can define its own kind | in progress | ✓ | — | **D153** (pending) · the measured finding it rests on: routing quality varied by KIND — vector had no cooldown or admission, score had no fallback at all |
