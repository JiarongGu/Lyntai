...
| 2026-09-21 | Affect as a memory axis — the measurement (oracle ceiling first, real annotator second) | not started | — | ✓ | nothing yet — `TASKS.md` Part 267 holds the open item; the plan is the execution route for a fresh session |
| 2026-09-21 | `Lyntai.Storage.FileSystem` — roster-first implementation plan | not started | — | ✓ | nothing yet — `TASKS.md` Part 268 holds the open item and the two REJECTED shapes; the plan is the execution route |
