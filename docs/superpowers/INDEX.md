...
|---|---|---|---|---|---|
| 2026-09-17 | The namespace restructure: one home for calling a backend | not started | ✓ | — | `TASKS.md` Part 103 (NS-1…NS-5, sequenced — the ORDER is the point) · the five measured problems, including `Lyntai.Inference` being a third lifecycle and `Lyntai.Providers` meaning three things |
| 2026-09-17 | The provider seam: one interface per SIGNATURE over a generic routed base, so a consuming app can define its own kind | 5 of 7 steps | ✓ | — | **D153** · the measured finding it rests on: routing quality varied by KIND — vector had no cooldown or admission, score had no fallback at all · what is left is `TASKS.md` Part 103 |
