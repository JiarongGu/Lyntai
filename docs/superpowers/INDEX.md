...
|---|---|---|---|---|---|
| 2026-09-27 | Vector read-back, filtered search and re-embedding a graph in place — design (an optional read interface, an id-set filter with a default body, a re-embed serialized against removals, chosen over metadata, an embed cache and a shadow vector space) and its four-task plan | Unreleased | ✓ | ✓ | **D194** (the rules and the rejected shapes) · `docs/task-archive.md` Part 323 |
| 2026-09-26 | Run-time text providers and front-door call tracing — design (a registry snapshot the DEFAULT router reads, chosen over a governed client per edit; tracing as a front-door decorator at slot 30, deterministic scorers by default) and its four-task plan | Unreleased | ✓ | ✓ | **D193** (the rules and the rejected shapes) · `docs/task-archive.md` Part 322 |
| 2026-09-26 | Run-time job schedules and coded job messages — design (a schedule store over the key-value store, one read/write interface; a `JobMessage` whose store members are required) and its seven-task plan | Unreleased | ✓ | ✓ | **D192** (the rules and the rejected shapes) · `docs/task-archive.md` Part 321 |
| 2026-09-26 | SentencePiece tokenization for the ONNX provider — design (an owned Unigram tokenizer read from `tokenizer.json`, chosen over `Microsoft.ML.Tokenizers`, which reads only the `.model` two candidates do not ship) and its ten-task plan | Unreleased | ✓ | ✓ | **D191** (the rule, the rejected dependency, typed specials stay text) · `docs/task-archive.md` Part 319 · `docs/FIXES.md` (the RoBERTa-family window) · `pitfalls.md` (a quantized graph cannot carry a pinned figure) |
| 2026-09-26 | Per-request CLI tools — design (a request-aware provisioner seam taking a `CliToolRequest`, and a per-consumer tool map on the MCP host, chosen over a request field that would change meaning across a fallback) and its three-task plan | 3.4.0 | ✓ | ✓ | **D190** (the rule and the three rejected shapes) |
