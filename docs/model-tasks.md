...
> `AddOnnxCrossEncoderProvider` loads a cross-encoder in process and declares `ProviderKinds.Score`, which
...
3. **You can now reach it from configuration** — `AddMemoryScoringVerification` fills the verification
   ...
   `AddHttpProvider`, or an in-process ONNX cross-encoder through `AddOnnxCrossEncoderProvider`, which needs no
