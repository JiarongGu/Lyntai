...
| **v3.3.0** (2026-09-25) | **storage you can read, and ONE owner where there were two** (D166–D189): a file backend in `Lyntai.Storage.Basic`, which replaces `Lyntai.Storage.InMemory` (D171, D173, D174); a chat reads and writes its memory through one seam, and a remember reports what it wrote (D175, D188); live routing moves a whole route, provider and model (D176); a generation pipeline runs as a durable job behind one media-spend gate, with ComfyUI taking inputs and producing 3D (D180, D181, D185); one owner per decorator slot, and one shared source for the SQL adapters' plumbing (D182, D186, D187). Corroboration as verification, offline consolidation and affect were measured and refused (D167–D169) |
| **v3.4.0** (2026-09-25) | **what the consuming apps showed** (D190): a CLI spawn's tools chosen per consumer by configuration, deny-by-default included; an HTTP backend's probe asks its server and lists its models; a graph write reports whether it kept its subjects (D175); a bridge refuses a kind its delegates cannot serve (D147, Breaking); a stopped Automatic1111 WebUI is a down host (D31) |
