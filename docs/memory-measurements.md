...
## Results — 128 measured: 98 current · 3 superseded · 27 retracted
...
**22 of 128 rows measure the arm that actually SHIPS.** Every other row is a ladder
...
| ---: | --- | --- | ---: | ---: | :---: | --- |
| 182 | shipped limit 10, model-free ranking (each … | miss-decomposition | 140 relevant entr… | 100.0% | **ships** | CURRENT |
| 198 | `gemma3:4b` judge via `AddMemoryVerificatio… | miss | unstated | 0.2571 | — | CURRENT |
| 247 | a perfect-oracle judge over a full corpus r… | miss | one full corpus r… | −0.0857 / −0.22… | — | CURRENT |
| 257 | `gemma3:4b` on the Japanese hard case (one … | screen-verdict | 1 call per model,… | `[3,4]` — answe… | — | CURRENT |
| 282 | `gemma3:4b` resident as a judge co-tenant o… | vram-resident | unstated | ~4.4 GB residen… | — | CURRENT |
| 361 | `+sem+rel-only+hl512+rerank` — `bge-reranke… | evidence-hit@k | 200 | +5.0 | — | CURRENT |
| 377 | `+sem+rel-only+rerank` at the shipped `Head… | evidence-hit@k | 200 | 78.0% | — | RETRACTED |
| 405 | `lyntai+hl512+rerank` — same `bge-reranker-… | prefers-current | 70 knowledge-upda… | 86.8% (59/68) | — | CURRENT |
| 437 | `+sem+rel-only+judge+top20` — the same 4B j… | evidence-hit@k | 200 | 71.0% | — | CURRENT |
| 472 | `LAMAR-600m` Q8_0 (2026-07) as reranker, ag… | evidence-hit@k | 200 | 91.0% | — | CURRENT |
| 502 | `jina-reranker-v1-tiny-en` Q4_K_M (gpustack… | screen-verdict | 1 query × 4 docum… | 8/8 checks | — | RETRACTED |
| 510 | the same three GGUFs on `cross-encoder/ms-m… | screen-verdict | 1 query × 2 docum… | control 10/10; … | — | CURRENT |
| 601 | `cross-encoder/ms-marco-MiniLM-L6-v2`'s own… | screen-verdict | 1 query × 2 docum… | fp32 reproduces… | — | CURRENT |
| 647 | `ms-marco-MiniLM-L6-v2` int8 and fp32 ONNX … | evidence-hit@k | 200 | +3.0 (both expo… | — | CURRENT <!-- drift-ok: carried from this result's own line --> |
| 693 | `LAMAR-600m` Q5_K_M on one `llama-server --… | screen-verdict | 6 calls of 4 docu… | 9.42e-3 max dri… | — | CURRENT |
| 710 | four sub-500 MB GGUF rerankers plus two con… | screen-verdict | per model: 1 quer… | mMiniLMv2 Q8_0 … | — | CURRENT |
| 757 | the adopter's scoring-verification seam ove… | evidence-hit@k | 240 zh/en questio… | mMiniLMv2 199 f… | — | CURRENT |
| 777 | an adopting application's own segmentation … | evidence-hit@k | 60 facts written … | mMiniLMv2 with … | — | CURRENT |
| 815 | the shipped `LlmMemoryVerificationPolicy` o… | endorsement-rate | 70 knowledge-upda… | 36.2% | — | CURRENT |
| 908 | witness COUNTING as model-free verification… | paired-preference | 70 knowledge-upda… | 32 correct / 27… | — | CURRENT |
| 946 | `SalienceWeight = 0` (now the shipped defau… | miss | 10 seeds; 10/10 l… | −0.0530 | **ships** | CURRENT |
| 986 | the FIRST VERSION of the same sweep (verdic… | miss | unstated | 5/5 shapes bett… | — | RETRACTED |
| 998 | shipped salience vs a `SalienceOff` control… | miss | 30 seeds × 6 shap… | +0.0384 | — | RETRACTED |
| 1103 | `NW1.5` (shipped `NoveltyWeight = 1.5`) vs … | miss | 30 seeds × 6 shap… | +0.0018 | **ships** | CURRENT |
| 1141 | `NW0.5` — the best rung under `nomic-embed-… | miss | 30 seeds × 6 shap… | −0.0116 | — | RETRACTED |
| 1167 | `lyntai` (shipped defaults) | evidence-hit@k | 200 | 54.5% | **ships** | CURRENT |
| 1236 | `lyntai` (shipped defaults) as FIRST publis… | evidence-hit@k | 200 | 11.0% | — | RETRACTED |
| 1349 | `+sem+rel-only` (semantic seeds on, `Retrie… | evidence-hit@k | 200 | 63.5% | — | SUPERSEDED by L1440 |
| 1430 | `+forget0+oracle` (PERFECT judge) multi-hop… | evidence-hit@k | 200 | 16 points | — | RETRACTED |
| 1440 | `+sem+rel-only` under per-source seed fusio… | evidence-hit@k | 200 | 83.0% | — | SUPERSEDED by L1996 |
| 1514 | ranking × walk interaction, `lyntai` → `lyn… | token-f1 | 1,540 | +1.0 | — | CURRENT |
| 1516 | the same ranking × walk interaction measure… | token-f1 | 100 | +6.7 / +4.2 / +… | — | RETRACTED |
| 1715 | shipped headline derivation — `MemoryHeadli… | marker-survival | 5,882 LoCoMo turns | 5,882 of 5,882 … | **ships** | CURRENT |
| 1753 | fixed-slot content contrast `lyntai-2shot` … | token-f1 | 1,540 | +1.1 | — | RETRACTED |
| 1799 | `lyntai-fused-full` — `lyntai-fused`'s own … | token-f1 | 200 | +11.7 | — | CURRENT |
| 1849 | `lyntai-fused-full`, multi-hop category cel… | token-f1 | 200 (multi-hop ce… | 38.0 vs 37.8 | — | RETRACTED |
| 1859 | `lyntai-fused-3shot-full` (three-shot walk … | token-f1 | 1,540 | −0.4 | — | CURRENT |
| 1898 | `lyntai-fused-3shot-full` (+ the walk, 39.7… | token-f1 | 200 | +0.9 | — | RETRACTED |
| 1941 | `lyntai-fused-api` (fused ranking + `Memory… | token-f1 | 1,540 | −0.4 | — | CURRENT |
| 1996 | `+sem+rel-only` (best mechanical: semantic … | evidence-hit@k | 1,540 | 82.6% | — | CURRENT |
| 2030 | `+forget0+oracle` (PERFECT judge, no semant… | evidence-hit@k | 1,540 | 74.6% | — | RETRACTED |
| 2063 | `+sem+rel-only` (the arm that wins LoCoMo) … | prefers-current | 70 knowledge-upda… | −37.1 | — | CURRENT |
| 2146 | `+sem+forget2` | evidence-hit@k | 200 | 69.5% | — | CURRENT |
| 2200 | `+sem+forget0.5` — the top rung of a six-po… | evidence-hit@k | 200 | 80.5% | — | CURRENT |
| 2245 | `+sem+rel-only+oracle` (a PERFECT judge — a… | evidence-hit@k | 200 | 92.5% | — | CURRENT |
| 2281 | `+sem+rel-only+judge` (gemma3:4b, shipped `… | evidence-hit@k | 200 | 72.5% | — | SUPERSEDED by L2356 |
| 2356 | `+sem+rel-only+judge@40` (depth 40 = 2×) | evidence-hit@k | 200 | 84.0% | — | CURRENT |
| 2403 | `+sem+rel-only+judge+fuse` (same judge, sam… | evidence-hit@k | 200 | 83.0% | — | CURRENT |
| 2497 | `+sem+rel-only+judge+budget5` | evidence-hit@k | 200 | 76.5% | — | CURRENT |
| 2552 | `extract+forget0` — strong-CLI extracted fa… | prefers-current | 70 knowledge-upda… | 52.9% | — | CURRENT |
| 2633 | `shot-2` on the `--haystack` variant (61,18… | all-evidence-recall | 125 of 133 multi-… | +4.8 | — | CURRENT |
| 2659 | `shot-3` on the ORACLE variant | all-evidence-recall | 125 of 133 multi-… | +6.4 | — | RETRACTED |
| 2673 | `extract+forget0` — `gemma3:4b`-extracted f… | prefers-current | 70 knowledge-upda… | 53.0% | — | CURRENT |
| 2714 | `extract+reconcile` — ADD/UPDATE/DELETE at … | prefers-current | 25 | 75.0% | — | RETRACTED |
| 2789 | `+sem` — decay ON (the one-knob partner of … | prefers-current | 70 questions (68 … | 72.5% | — | CURRENT |
| 2832 | `RetrievabilityWeight` = 1 — shipped | recovery@k | 26 buried entries… | 100.0% | **ships** | CURRENT |
| 2865 | the FIRST `--recover` run — `page@10` repor… | recovery@k | unstated | 'decay deletes … | — | RETRACTED |
| 2885 | `+forget4` — `RetrievabilityWeight` walked … | prefers-current | 70 questions, of … | 100.0% | — | CURRENT |
| 2906 | `lyntai`, haystack variant (model-free, k =… | prefers-current | 70 knowledge-upda… | 86.4% | **ships** | CURRENT |
| 2977 | `lyntai` vs `vector` on LoCoMo, shared-stor… | evidence-hit@k | unstated | −49.5 | — | RETRACTED |
| 3017 | `lyntai` shipped ranking observed by the `-… | rrf-score-separation | 25 questions, sam… | −29% | **ships** | CURRENT |
| 3063 | K = 120 (RRF ladder rung), haystack | current@k | 25 questions | 0.0 points | — | RETRACTED |
| 3089 | K = 60 (shipped) | evidence-hit@k | 200 LoCoMo questi… | 54.5% | **ships** | CURRENT |
| 3136 | `shot-1` (single recall, no expansion), Lon… | clean | all 70 questions | 31.4% | **ships** | CURRENT |
| 3168 | `shot-1` on the 25-question sample of the s… | clean | 25 questions | 40.0% | — | RETRACTED |
| 3174 | `GraphMemoryOptions.ExpansionRetrievability… | clean | 25 questions | +4.0 | — | RETRACTED |
| 3254 | `shot-2` on LoCoMo, shared-store run (pre-i… | evidence-hit@k | 200 questions | +6.0 | — | RETRACTED |
| 3280 | `shot-1` (one-shot recall, shipped `k = 10`… | clean | 70 | 31.4% | — | CURRENT |
| 3345 | `fill` (`k = 80`, engine's own `CharBudget`… | clean | 70 | 57.1% | — | CURRENT |
| 3387 | `fill` (`k = 80`) latency in the run that p… | latency | unstated | 14.7 seconds | — | RETRACTED |
| 3391 | `pool-16` (`GraphMemoryOptions.CandidateMul… | clean | 70 | 58.6% | — | CURRENT |
| 3431 | `CandidateMultiplier` 4 (shipped) → 16, on … | all-evidence-recall | unstated | −28.0 | — | CURRENT |
| 3457 | shipped `CandidateMultiplier = 4` (the `lyn… | evidence-hit@k | 200 | 54.5% | **ships** | CURRENT |
| 3511 | `+sem+rel-only+oracle+pool16` (×16 pool = 3… | evidence-hit@k | unstated | 96.0% | — | CURRENT |
| 3537 | `+oracle+pool32` (640 candidates, larger th… | evidence-hit@k | unstated | 100.0% | — | RETRACTED |
| 3550 | `Partition` (shipped) against `+oracle+fuse… | evidence-hit@k | 200 | +2.0 | — | CURRENT |
| 3576 | `ExpansionRetrievabilityFloor = 0.8`, knowl… | clean | 70q (knowledge-up… | +2.8 | — | CURRENT |
| 3686 | shipped novelty salience policy at the ship… | miss | 10 seeds × 4 shap… | 0.710 | **ships** | CURRENT |
| 3725 | `SalienceContext.SimilarCount` on authored … | separability-auc | 5 recurrence / 5 … | AUC = 1.000 | — | CURRENT |
| 3757 | `recurrence` population mean `SimilarCount`… | similar-count | one probe per pop… | 6.00 | — | RETRACTED |
| 3778 | `sum` — Σ r(m) — over an unbuilt gist tier,… | regime-picks | 600 replays = 60 … | phase A 300/300… | — | CURRENT |
| 3809 | `count@θ`, θ = 0.9 read as one of the two p… | regime-picks | 600 replays = 60 … | phase B 300/300… | — | RETRACTED |
| 3890 | `count@0.9` across `RoutineCount` rungs 3 /… | regime-picks | 2400 replays = 60… | tie 185 → A 175… | — | CURRENT |
| 3930 | `mean` — Σ r(m)/n — under `bulk` with `Corp… | regime-picks | 2400 replays per … | B 300/300 at se… | — | CURRENT |
| 3984 | reinforce on **expansion only** (the shippe… | miss | unstated | 0.4429 | — | CURRENT |
| 4001 | `DsrOptions.ReinforceGain = 0` (shipped) ag… | miss | the fixed corpus … | 0.234 → 0.103 | **ships** | CURRENT |
| 4011 | `ReciprocalRankFusionOptions.DiagnosticityW… | miss | 20 seeds × 4 shap… | `topical` 0.059… | — | CURRENT |
| 4023 | shipped novelty-driven salience policy, iso… | miss | unstated (the `ma… | −0.0786 | **ships** | CURRENT |
| 4066 | a two-character `ILIKE` against a three-cha… | latency | 300k rows, one pa… | 96.6 ms against… | **ships** | CURRENT |
| 4074 | `shot-2` on the `--haystack` variant, `sing… | all-evidence-recall | 64 of 70 single-s… | +0.0 | — | CURRENT |
| 4117 | `+sem+rel-only+judge` with `gemma-3-1b-it` … | evidence-hit@k | 200 | 83.0% | — | CURRENT |
| 4161 | `rerank` — the bench-local `CrossEncoderVer… | latency | 50 writes + 50 re… | 94.5 ms recall … | — | CURRENT |
| 4260 | `+sem+rel-only+judge` — the SHIPPED `Memory… | token-f1 | 301 questions sam… | −0.0346, CI [−0… | **ships** | CURRENT |
| 4303 | `+sem+rel-only+judge+enginefuse` — the same… | token-f1 | 301 questions sam… | +0.0295, CI [+0… | — | CURRENT |
| 4374 | the first `memory-decision` grid — gold tak… | forced-choice-accuracy | 200 trials x 5 li… | rerank 53.5% at… | — | RETRACTED |
| 4382 | the same grid's generative tie counts, take… | forced-choice-accuracy | 200 trials x 5 li… | score-4b 100/20… | — | RETRACTED |
| 4395 | `rerank` — a 468,393,760 B cross-encoder ar… | forced-choice-accuracy | 261 trials x 5 li… | 72.0% vs 71.5% … | — | CURRENT |
| 4445 | `score-4b` — how often a generative 0-100 s… | forced-choice-accuracy | 261 trials x 5 li… | score-4b ties o… | — | CURRENT |
| 4498 | `loop-4b` — a 2,489,757,856 B instruct mode… | forced-choice-accuracy | 168 trials x 5 ro… | 86.3% vs 81.0% … | — | CURRENT |
| 4511 | `loop-1b` — a 806,058,240 B instruct model … | forced-choice-accuracy | 168 trials x 5 ro… | 10.1% at N=3 fa… | — | CURRENT |
| 4548 | `loop-4b` on 20 requests NO tool in the ros… | false-call-rate | 20 negative reque… | 90-95% shipped;… | **ships** | CURRENT |
| 4582 | four sub-100 MB GGUF embedders — `all-MiniL… | screen-verdict | 1 known-similar/k… | 4 of 4 HEALTHY;… | — | CURRENT |
| 4612 | `cosine-minilm` — a 25,008,064 B `all-MiniL… | forced-choice-accuracy | 168 trials x 5 ro… | 78.6% at N=3 an… | — | CURRENT |
| 4693 | `potion-base-8M` (30,236,760 B, a CPU looku… | evidence-hit@k | 200 | 54.0% against 5… | — | CURRENT |
| 4738 | the `model2vec`/`potion` static family — a … | forced-choice-accuracy | 168 trials x 5 ro… | potion 52.4-70.… | — | CURRENT |
| 4770 | subtracting the corpus centroid before cosi… | forced-choice-accuracy | 168 trials x 5 ro… | +8/+6/+3 for th… | — | CURRENT |
| 4796 | `lyntai-fused-full` against `lyntai-fused` … | token-f1 | 61 questions x 2 … | completeness +7… | **ships** | CURRENT |
| 4846 | `nomic-embed-text-v1.5` Q8_0 (146,146,432 B… | token-f1 | 61 questions samp… | every arm withi… | — | CURRENT |
| 4895 | `loop-tool-native` — `qwen2.5-0.5b-instruct… | forced-choice-accuracy | 168 trials x 5 ro… | -2.4 to -9.6 po… | — | CURRENT |
| 4903 | `qwen2.5-0.5b-instruct` Q4_K_M against `gem… | screen-verdict | 2 models x 3 tool… | qwen 6/6 emit t… | — | CURRENT |
| 4980 | `loop-tool-native` against `loop-tool`, the… | false-call-rate | 20 negative reque… | 20-30% native a… | — | CURRENT |
| 5017 | `full` — the shipped recall at k = 10 and t… | clean | 70 questions, hay… | -11.4 at 1,200;… | — | CURRENT |
| 5073 | `full` — the shipped recall asking for Memo… | clean | 70 questions, hay… | a monotone rise… | — | CURRENT |
| 5119 | `full` — the shipped recall asking for Memo… | all-evidence-recall | 132 questions, ha… | negative at eve… | — | CURRENT |
| 5162 | `cosine` — a 333,590,944 B embedder scoring… | forced-choice-accuracy | 168 trials at eac… | 96.4 percent at… | — | CURRENT |
| 5199 | the SHIPPED `LlmMemoryAnnotationPolicy` aga… | drift-rate | 24 eligible facts… | 41.7% to 87.5%;… | — | RETRACTED |
| 5241 | a pure-CODE reconciler over the SAME model … | drift-rate | 24 eligible facts… | no change in 5 … | — | CURRENT |
| 5280 | the same three models and fixture through t… | drift-rate | 20-24 eligible fa… | 83.3% to 90.5% … | — | CURRENT |
| 5307 | a pure-CODE recency rule — carry the PREVIO… | drift-rate | 20-24 eligible fa… | gap 0: 87.0% → … | — | CURRENT |
| 5337 | a bench-local selective annotator — number … | drift-rate | 4-24 eligible fac… | 0.0% drift at 1… | — | CURRENT |
| 5380 | an OFFLINE pass over the stored graph — cos… | separability-auc | 32 facts and 48 s… | cosine AUC 0.77… | — | CURRENT |
| 5426 | per-turn AROUSAL as a predictor of later re… | separability-auc | 5,882 LoCoMo turn… | 0.474 within le… | — | CURRENT |
| 5463 | the graph engine on SQLite at shipped defau… | latency | 1k / 10k / 100k e… | recall p50 10.4… | **ships** | CURRENT |
| 5478 | `shipped` and `read-only` at 1k, 1 to 8 wor… | latency | five repeats per … | `shipped` p99 4… | **ships** | CURRENT |
| 5499 | the same recalls with `SqliteRuntime.Disabl… | latency | 1 to 16 workers; … | 340 → 4,665 rec… | — | CURRENT |
| 5523 | `shipped` minus `read-only` p50 at 1k / 10k… | latency | five repeats per … | 76% of p50 at 1… | **ships** | CURRENT |
| 5542 | substring recall by SCANNING — enumerate a … | latency | 1,000 / 10,000 / … | 1,898 ms p50 at… | — | CURRENT |
| 5565 | the shipped `WriteBackAsync` of the file gr… | latency | 300 repeats per o… | 7.15–7.39 ms p5… | **ships** | CURRENT |
...
#### Rescue depth SATURATES at four times the limit — with a PERFECT judge (2026-08-13) <!-- result: id=judge-oracle-depth-saturation arm="a perfect-oracle judge over a full corpus replay at limit 10, `VerificationDepth` 10 / 20 / 40 / 80 / 160 / 5000" metric=miss n="one full corpus replay per depth" value="−0.0857 / −0.2214 / −0.2500 at depth 10 / 20 / 40; 80 and beyond identical to 40" ships=no status=CURRENT -->

Depth 10 is observe-only — the judge sees exactly the page — and recovers `-0.0857` of the miss rate; 20
recovers `-0.2214` and 40 `-0.2500`, and 80, 160 and 5000 all return exactly what 40 did. That knee is what
`GraphMemoryOptions.DefaultVerificationDepthFactor` (4 × the limit) encodes (**D59**): past it, depth costs
tokens linearly and rescues nothing. **It is an ORACLE's curve**, and an oracle never endorses junk, so for
it depth is free; for a real judge depth is a PRECISION trade — the same 4B model is level with no judge at
2× and costs 10.5 points at the shipped 4× on LoCoMo (`locomo-judge-depth40-n200`,
`locomo-judge-4b-depth80-n200`). Read the knee as a ceiling on the MECHANISM, never as a fit for a real one.
...
#### An ADOPTING APPLICATION's screen on b10549: modern-bert loads, a 132.6 MB multilingual reranker scores correctly, and the reference pair cannot catch an overlap-ranker (2026-09-24) <!-- result: id=rerank-screen-adopter-b10549 arm="four sub-500 MB GGUF rerankers plus two controls, each on its own `llama-server --reranking` (llama.cpp b10549, commit b2e5e9b28), screened by an adopting application on this file's reference pair and on its own overlap-trap pair" metric=screen-verdict n="per model: 1 query × 2 documents (reference) + 1 query × 2 documents (overlap trap), plus one over-long input" value="mMiniLMv2 Q8_0 passes both; xVITA passes the reference pair and FAILS the trap; modern-bert loads" ships=no status=CURRENT -->

**Measured by an adopting application, not here — and the screen half is reproducible anyway**: every file is
public and named by uploader and sha256 below, and the reference pair is this file's own
(`rerank-screen-reference-pair`). The overlap trap is the adopter's own two-document pair, where the
distractor shares MORE of the query than the answer does; its text is theirs and is not reproduced here.
Spread ratios are converted to this file's convention (published 12.9272 ÷ observed).

| model | uploader · file | on disk | sha256 | arch · `token_type_count` | reference pair | spread | overlap trap |
|---|---|---:|---|---|---|---|---|
| `bge-reranker-v2-m3` Q5_K_M (control) | gpustack · `bge-reranker-v2-m3-Q5_K_M.gguf` | 468,392,352 B | `1a2120…813ef` | `bert` · 1 | 5.784 / −8.213 ✓ | 13.997 (0.92×) | ✓ |
| `LAMAR-600m` Q5_K_M (control) | mradermacher · `LAMAR-600m.Q5_K_M.gguf` | 468,393,760 B | `ec708b…a546f` | `bert` · 1 | 6.159 / −7.707 ✓ | 13.866 (0.93×) | ✓ |
| **`mmarco-mMiniLMv2-L12-H384-v1` Q8_0** | keisuke-miyako · `mmarco-mMiniLMv2-L12-H384-v1-Q8_0.gguf` | **132,584,000 B** | `91d703…26eed` | `bert` (XLM-R) · 1 | 10.713 / −4.176 ✓ | 14.889 (0.87×) | ✓ |
| `xVITA-Rerank-300M-zhTW` Q8_0 | xCloudinfo · `xVITA-Rerank-300M-zhTW-Q8_0.gguf` | 332,894,432 B | `f894c7…9faf9ef1` | `modern-bert` · absent | 3.386 / −5.605 ✓ | 8.991 (1.44×) | **✗** |
| `Qwen3-Reranker-0.6B` Q6_K (reference) | Voodisss · `Qwen3-Reranker-0.6B-Q6_K.gguf` | 494,879,136 B | `5916ab…e4b99e` | `qwen3` · absent | 0.9985 / 0.0015 ✓ | probabilities | ✓ |

Full digests: `1a212007526c7083627eed92b39dd4472e90ff1374a03fb068733378220813ef`,
`ec708b20336577c63702dd8efb23060bc611933579572bf9ad47ce2eaeda546f`,
`91d70301828ba735c22eda56adb649f48975f371337e8c8b046326b885e26eed`,
`f894c75201a42073ab71c678a21b56b74a9fa16dee31069e51ece7b50faf9ef1`,
`5916ab2926388177a0a7b6eedfd909d214a29592ce35153a7d9ff86c08e4b99e`. The mMiniLMv2 repository is pinned at
commit `2b37d162c88e0aeb8a1b4acb2d50f0e5ade16fd5`.

**Four findings, each answering a question this file or `docs/model-tasks.md` §3.2 had left open.**

1. **`modern-bert` LOADS on llama.cpp b10549** — the open question for `xVITA-300M`. It carries
   `cls.weight`, `cls.norm.weight` and `cls.output.weight/bias`, and no `token_type_count` at all.
2. **The reference pair CANNOT catch a model that ranks by word overlap.** `xVITA` passes it at 1.44× and
   its own model card's example ranks correctly, yet on the trap it scores the distractor **10.019 against
   the answer's 7.420** — 10.727 against 7.128 in Traditional characters, and swapping the document order
   changes neither. This is the retracted four-document fixture's lesson one level up: a screen that one
   broken model passes is too easy for that model's failure.
3. **A correct MULTILINGUAL reranker exists at 132,584,000 B** — `mmarco-mMiniLMv2-L12-H384-v1` Q8_0,
   XLM-R and therefore immune to #21729, passing both pairs. That is **3.5× below** the 468,393,760 B
   this file had recorded as the multilingual floor, and within 6.1% of the 124,925,504 B that the
   vocabulary argument above says no quant can beat. Its training data is mMARCO, whose MS MARCO source is
   licensed non-commercially, and the GGUF repository declares no licence.
4. **The 512-token window rejects the WHOLE call.** One over-long document gets
   `400 input (1052 tokens) is larger than the max context size (512 tokens). skipping` — the wording
   `rerank-screen-reference-pair` captured on b10603 — and no document in the batch is scored. Slots cap at
   512 whatever `--ctx-size` says. Dense Chinese at a 1,000-character cap is 781 tokens, and on this
   tokenizer tokens ≤ characters + 1 across the adopter's adversarial strings, so a CHARACTER cap is a hard
   bound for it.

**The controls reproduce across builds**: `LAMAR-600m` reads 6.159 / −7.707 here against 6.161 / −7.700 on
b10603 — inside the 9.42e-3 drift `rerank-repeatability-drift` measured on one server.

#### …and on the adopter's own fixture, the 132.6 MB model trails LAMAR on found@8 and leads on top-1 (2026-09-24) <!-- result: id=rerank-bench-adopter-zh-en-240 arm="the adopter's scoring-verification seam over each reranker, EndorseCount 8 = the page, at most 60 candidates, NO embedder, against a formula base of 125 found@8 / 79 top-1" metric=evidence-hit@k n="240 zh/en questions over 60 facts (an adopting application's fixture; the longest fact 101 characters)" value="mMiniLMv2 199 found@8 against LAMAR 208 (−3.8 pp, p = 0.004) and BGE 204 (n.s.)" ships=no status=CURRENT -->

**A corpus this repository does not have**, so read the DIRECTION and treat every magnitude as the adopter's
(`.claude/knowledge/model-decoupling.md`). No length cap bound — the longest fact is 101 characters — so the
window finding above plays no part.

| arm | found@8 | top-1 | serial median |
|---|---:|---:|---:|
| formula base | 125 | 79 | — |
| `bge-reranker-v2-m3` Q5_K_M | 204 | 90 | 447 ms |
| `LAMAR-600m` Q5_K_M | **208** | 86 | 498 ms |
| `mmarco-mMiniLMv2` Q8_0 | 199 | **99** | **313 ms** |
| `Qwen3-Reranker-0.6B` Q6_K | 207 | 92 | 1,530 ms |

**The small model is significantly behind LAMAR on found@8** (−3.8 pp, discordant 9/0, p = 0.004), **not
distinguishable from BGE** (−2.1 pp, 95% [−4.6, +0.5], discordant 7/2, p = 0.180 — and not shown
equivalent either), and **first on top-1** while being the fastest arm. Every arm returned a verdict on all
234 graph recalls. `xVITA` was not benched: it failed the trap. `ships=no` — nothing here recommends a
default, and the licence caveat above travels with any recommendation that does.

#### SEGMENTED, a 512-window reranker finds an answer at the END of a long note: 4 → 44 of 60, where cutting read worse than no judge (2026-09-24) <!-- result: id=rerank-segmented-adopter-long-notes arm="an adopting application's own segmentation (a 506-character pair window less the query, the query capped at 253, a document scored as its best window) against cutting at the window, over mmarco-mMiniLMv2 Q8_0, bge-reranker-v2-m3 and LAMAR-600m on llama.cpp b10549" metric=evidence-hit@k n="60 facts written out as ~900-1,200-character notes, the answer at the start, middle, end or beyond 1,000 characters; 240 zh/en questions, EndorseCount 8 (the page), no tags, no embedder" value="mMiniLMv2 with the answer at the end: found@8 4 → 44 of 60 (discordant 0/40, p < 0.001)" ships=no status=CURRENT -->

**Measured by an adopting application, on a corpus this repository does not have**, so read the DIRECTION and
treat every magnitude as the adopter's (`.claude/knowledge/model-decoupling.md`). It built its own
segmentation before **D177** and measured it against cutting on the fixture the result above describes, with
each of its 60 facts written out as a note of ~900–1,200 characters and the answer placed at the start, in the
middle, at the end, or beyond 1,000 characters. Same 240 questions, no tags, no embedder, EndorseCount 8 (the
page), llama.cpp b10549.

| reranker | where the answer sits | found@8, cut | found@8, segmented |
|---|---|---:|---:|
| `mmarco-mMiniLMv2` Q8_0 | end | 4 | **44** |
| `mmarco-mMiniLMv2` Q8_0 | middle | 8 | **50** |
| `bge-reranker-v2-m3` Q5_K_M | beyond 1,000 characters | 3 | **51** |
| `LAMAR-600m` Q5_K_M | beyond 1,000 characters | 3 | **52** |
| no judge | end | 29–30 | — |

Of 60 facts each. With the answer at the end, mMiniLMv2's pairs are discordant 0/40, p < 0.001.

1. **Cut, a reranker that cannot see the answer reads WORSE than no judge** — 4 against 29–30 — because under
   the partition its endorsements displace the base order. Segmenting is what lets a 512-window reranker
   verify a long entry at all.
2. **With the answer at the START there is no significant loss, but a descriptive 8 losses to 1 gain**, most
   likely later windows of OTHER notes outscoring the answer: a document scored as its best window gets one
   chance per window. That is **D177**'s recorded known limit.
3. **Short facts (≤ 101 characters) are byte-identical under both**, which is D177's promise for an input
   within its bound.

**What it costs.** Serial median latency on the long notes, cut → segmented: BGE 2.0 → 3.2 s, LAMAR 2.2 → 3.2 s,
mMiniLMv2 0.5 → 1.2 s. And one call that segmented into 2,000 windows took 77–79 s on an RTX 4080 Laptop GPU,
480 windows ~20 s — the case `InputSegmentation.MaxPiecesPerInput` bounds per input. `ships=no`: segmenting is
configured, never a default.

**Why the character bound counts after NFKC — the same adopter's tokenizer sweep.** On an XLM-R tokenizer,
...
#### Counting witnesses is the obvious FREE verification, and there is nothing to count (`memory-longmemeval --corroborate`, 2026-09-21) <!-- result: id=longmemeval-corroboration-flat-n70 arm="witness COUNTING as model-free verification — embedding-cosine and token-Jaccard detectors over unflagged user turns, both corpus variants" metric=paired-preference n="70 knowledge-update questions, haystack (34,242 turns per arm); the oracle variant agrees cell for cell" value="32 correct / 27 backwards / 11 flat at cosine 0.70 — indistinguishable from a coin" ships=no status=CURRENT -->

The field's cheap alternative to a judge gates the WRITE: a claim persists only after two independent
sources and three separated re-assertions, no model in the loop (the Ombre-Brain comparison named it;
`docs/task-archive.md` Part 270 closed it). This run asks the PRIOR question — does the corroboration signal EXIST on the
workload where the judge fires backwards (**D109**, above)? No arms and no ingestion: a witness is an
UNFLAGGED user turn similar to a claim's flagged turn(s), counted by two detectors so the answer does not
rest on one. `nomic-embed-text` on `llama-server` (port 8091, its own instance), cosine θ ∈ {0.60, 0.70,
0.80}; content-token Jaccard ∈ {0.35, 0.50}.

| θ (cosine) | witnesses/q cur | witnesses/q sta | cur&gt;sta | sta&gt;cur | tie | gate cur | gate sta |
|---|---|---|---|---|---|---|---|
| 0.60 | 7.23 | 6.77 | 32 | 27 | 11 | 90% | 83% |
| **0.70** | 2.50 | 2.34 | **32** | **27** | **11** | 36% | 33% |
| 0.80 | 0.59 | 0.64 | 19 | 23 | 28 | 10% | 9% |

**Three findings, and each kills a different branch of the proposal.**

- **The paired signal is FLAT.** 32/27/11 is a coin (the judge's cells, for contrast: 5/23/35 — backwards).
  The two facts' best-witness similarity distributions are indistinguishable — p50 0.769 (current) against
  0.779 (superseded) — so no threshold exists that separates them: this is not a tuning miss.
- **The counts are mostly NOISE.** An arbitrary unflagged user turn collects 1.81 witnesses at θ = 0.70 (700
  probes); the facts collect ~2.4. The excess over noise is roughly half a witness — and equal on both
  sides. The model-free detector says the same thing more bluntly: at Jaccard ≥ 0.50, 65 of 70 questions
  have ZERO witnesses on either side. A conversational corpus does not re-assert facts often enough to
  count, which is Part 166's supersession-density finding arriving by a second route.
- **The persistence gate REFUSES the answer.** At the field's own constants (≥3 assertions across ≥2
  sessions) the CURRENT fact — the one the question is scored on — passes 36% of the time at θ = 0.70.
  Loosening to θ = 0.60 passes it 90%, and passes the superseded fact 83%: the gate opens by admitting
  noise, never by finding signal.

**What this does NOT say.** It prices witness counting on ONE workload, chosen because it is the workload
this subsystem makes its supersession claim on and the one where the judge already failed. A corpus with
genuine multi-source structure — several writers, a feed with repetition — could still power the gate; what
is refuted is counting witnesses over a single user's conversation, and the write-persistence constants as
a filter anywhere recall is scored on facts stated once. Both corpus variants and both detectors agree, so
reviving this takes a corpus with measured re-assertion density, not another threshold.
...
**83.0% is a SQLite number, and two of four shipped backends never reach this path.**
`InMemoryMemoryGraphStore` reports a flat `Relevance` for every lexical match and `Matched null` for every
subject fetch, and the file graph store shares its core (`docs/DECISIONS.md` D174), so under the shipped
default registration (lexical + subject, no semantic) no candidate carries a rank and the recall runs the
pre-branch pooled fallback — this measurement never touches either store. `MultiplicativeRankingPolicy` still ranks on the pooled `GraphNode.Relevance` field, so the
...
### Stability growth OFF wins on every corpus shape: `ReinforceGain = 0` (2026-08-12) <!-- result: id=reinforce-gain-zero-fixed-pin arm="`DsrOptions.ReinforceGain = 0` (shipped) against the compounding growth it replaced, on the fixed-corpus pin" metric=miss n="the fixed corpus `MemoryDefaultRecallQualityTests` replays; six shapes across thirty paired seeds for the ladder" value="0.234 → 0.103" ships=yes status=CURRENT -->

The fixed-corpus pin (`MemoryDefaultRecallQualityTests`) moves miss **`0.234 → 0.103`**, roughly 56%
relative, without paying for it in pollution, and the change wins on all six corpus shapes across thirty
paired seeds. **Every alternative was built and lost** — the compounding rule, a CAPPED variant, and one
computed from the entry's recall COUNT so it could not compound by construction — which rules out
"compounding is the problem" as the explanation. **D54** records the decision; what fits is that the age
reset EXPIRES while growth PERSISTS and is conditioned on the ranker's own output, so it banks the ranker's
errors.

### ACT-R's fan effect makes recall WORSE on a co-activation graph (`memory-fan`, 2026-08-15) <!-- result: id=fan-diagnosticity-weight-ladder arm="`ReciprocalRankFusionOptions.DiagnosticityWeight` at 0 / 0.5 / 1 / 2 (it ships at 0)" metric=miss n="20 seeds × 4 shapes" value="`topical` 0.059 → 0.064 → 0.131 → 0.320" ships=no status=CURRENT -->

`node devtools/dev.mjs memory-fan`, 20 seeds × 4 shapes, weights 0 / 0.5 / 1 / 2:
...
Most edges here come from CO-ACTIVATION, so a node's degree also measures how often it was useful, and a
degree penalty penalises exactly what a caller keeps coming back to — a confound written into the sweep's
own `NOT swept` block before the run. **D62** keeps the knob at 0 and states the bar for re-proposing it: an
annotation-dominated graph, measured.
...
#### A two-character pattern cannot use the trigram index: ~108× on Postgres (2026-08-12) <!-- result: id=postgres-bigram-ilike-seqscan arm="a two-character `ILIKE` against a three-character one on identical, equally selective data — Postgres, `pg_trgm` GIN index" metric=latency n="300k rows, one pattern each" value="96.6 ms against 0.90 ms" ships=yes status=CURRENT -->

Most Chinese content words are two characters, below the trigram floor, so a substring backend must carry
them (**D55**) — and on Postgres a two-character `ILIKE` cannot use the `pg_trgm` GIN index and degrades to
a parallel sequential scan: **96.6 ms against 0.90 ms** for a three-character pattern. That is why the
widened clause runs only after the index-friendly pass returns nothing, and why removing the staging would
be a silent performance regression rather than a simplification.
...
### REFUTED: an offline consolidation pass finds nothing the write path could not — its similarity half IS a write-time knob, and graph structure adds only noise (`memory-consolidation`, 2026-09-23) <!-- result: id=consolidation-offline-pass arm="an OFFLINE pass over the stored graph — cosine threshold, common neighbours, token overlap — proposing pairs the write path left unlinked, on an engine WITH an embedder and no annotator (`nomic-embed-text`, unprefixed, `llama-server` on its own port)" metric=separability-auc n="32 facts and 48 same-entity pairs per language (8 entities x 4 facts, interleaved at gap 7), English and Chinese" value="cosine AUC 0.773 English, 0.519 Chinese over unlinked pairs; no pre-registered row clears 2x base rate AND 25% of headroom in either language" ships=no status=CURRENT -->

Ombre-Brain's `dream` runs a pass over stored memory and emits connections that no write made
(`docs/task-archive.md` Part 271 closes the item). The question asked before building one: does a pass over
`IMemoryGraphStore` find same-entity links the write path missed? The fixture is
`memory-annotation-drift`'s — the first fact names an entity and the rest refer to it obliquely — and never
`MemoryCorpus`, whose attribute cluster shares a template suffix a similarity pass would link through. The
baseline is the engine with an embedder and NO annotator, which is the most headroom consolidation is ever
offered: a real annotator only removes some. **Pre-registered**: a detector passes at precision ≥ 2× the
base rate AND ≥ 25% of the headroom recovered, the same row in both languages.

| detector (unlinked pairs only) | EN proposed / true | EN precision (lift) | EN recovered | ZH proposed / true | ZH precision (lift) | ZH recovered |
|---|---|---|---|---|---|---|
| cosine ≥ 0.60 — the write floor | 0 / 0 | — | 0% | 148 / 17 | 11.5% (1.12×) | 49% |
| cosine ≥ 0.70 | 0 / 0 | — | 0% | 25 / 4 | 16.0% (1.56×) | 11% |
| cosine ≥ 0.45 — *post-hoc* | 85 / 22 | **25.9% (2.66×)** | **46%** | 342 / 35 | 10.2% (1.00×) | 100% |
| common neighbours ≥ 1 | 1 / 0 | 0.0% | 0% | 312 / 31 | 9.9% (0.97×) | 89% |
| common neighbours ≥ 2 | 0 / 0 | — | 0% | 245 / 27 | 11.0% (1.08×) | 77% |
| token Jaccard ≥ 0.20 | 3 / 0 | 0.0% | 0% | 0 / 0 | — | 0% |

Base rate 9.7% English, 10.2% Chinese; cosine ≥ 0.80 and Jaccard ≥ 0.35 proposed nothing anywhere. The
write path itself linked 2 English pairs (neither same-entity) and 154 Chinese ones at 8.4% precision —
BELOW the base rate, so with this embedder Chinese similarity linking is noise at write time too.

- **The cosine half is a KNOB, and that is structural rather than measured.** For any pair, the later write
  already saw the earlier one, so `SimilarityK` ≥ the store size with `MinSimilarity` = θ reproduces any
  offline threshold pass exactly. The one row that clears the bar — English at 0.45, added post-hoc because
  the whole English distribution sits below the write floor (same-entity p50 0.448 against 0.387) — is
  therefore `MinSimilarity`'s question, which `docs/memory.md` already frames as a trade, never a seam's.
  The 148 Chinese pairs at 0.60 are exactly what the write path's cap cut, at base-rate precision.
- **Structure — the one thing only an offline pass can see — is noise.** Common neighbours sits at 0.97–1.08×
  over the Chinese similarity graph and has nothing to close over in English. On top of a PERFECT annotator
  every proposal is false: 13 English and 303 Chinese at ≥ 1, because a subject clique is already closed and
  closure can only cross it.
- **Chinese carries no cosine signal at all** (AUC 0.519), so no threshold rescues it on this embedder.

**Controls, declared before the first run, all passed in both languages**: the perfect-annotator arm reads
headroom 0; every write-time edge clears 0.60 by `VectorMath.Cosine`, so the bench sees the engine's
vectors; every edge reads back symmetric. A second run reproduced every pre-registered row exactly.

**What this does NOT say.** Nothing about recall quality — an edge found is not a recall improved. Nothing
about a MODEL-driven pass that reads a window and names connections: that is annotation with more context,
and the drift record above says subject linking is bound to the model rather than to code. One embedder,
unprefixed (the bench-wide caveat `locomo-qa-second-embedder` records), synthetic, 48 pairs a
language — read directions. `ships=no`; nothing changed, and **D168** records the refusal.

### REFUTED: arousal does not predict what gets asked about later — within length it sits at chance, and its raw signal is LENGTH (`memory-affect`, 2026-09-23) <!-- result: id=affect-arousal-predicts-evidence arm="per-turn AROUSAL as a predictor of later relevance — `gemma-3-4b-it` Q4_K_M rating 1-9 on `llama-server` (its own port) and the NRC-VAD v2.1 lexicon — LoCoMo turns cited as evidence against the rest, within length quartiles" metric=separability-auc n="5,882 LoCoMo turns, 1,424 cited as evidence by 1,540 category 1-4 questions" value="0.474 within length quartiles (raw 0.544; length alone 0.724)" ships=no status=CURRENT -->

Ombre-Brain carries valence and arousal per memory, makes arousal a retention multiplier and surfaces
unresolved high-arousal material with no query (`docs/task-archive.md` Part 273 closes the item). An affect
ORACLE would re-measure `importance-novelty-critical-rare` — a perfect importance signal through salience is a
redistribution — and on authored fixtures its author would decide whether arousal marks what gets asked. So
the question asked is the empirical one: **does arousal PREDICT later relevance on real conversations?** A
turn is positive when a category 1–4 LoCoMo question cites it as evidence. Two detectors: a chat model
rating 1–9 at temperature 0, and the NRC-VAD v2.1 unigram lexicon (Saif M. Mohammad, National Research
Council Canada, 2025 — <https://saifmohammad.com/WebPages/nrc-vad.html>; research use, never committed).
**Pre-registered**: model arousal must reach AUC ≥ 0.60 within length quartiles.

| signal | AUC | AUC within length quartiles | evidence rate in its top tenth |
|---|---|---|---|
| model arousal | 0.544 | **0.474** | 30.4% (1.26×) |
| model \|valence − 5\| | 0.494 | 0.448 | 23.6% (0.98×) |
| lexicon arousal (mean) | 0.481 | 0.470 | 16.0% (0.66×) |
| length (chars) — the control | **0.724** | 0.571 | 53.9% (2.23×) |

Base rate 24.2%. **The raw arousal signal is length**: 0.544 overall becomes 0.474 once turns are compared
with turns of their own length, and length alone reaches 0.724. Within length, evidence turns are if
anything CALMER and less emotionally extreme than their neighbours — LoCoMo's questions ask after facts
(when, where, who), and the turns that carry them are statements, while the aroused ones are reactions.
Both detectors agree, so the answer rests on neither.

**Controls, declared before the run, all passed**: the model separated five calm from five intense fixed
sentences at AUC 1.000 (the lexicon also 1.000), so this is a rater that CAN rate; it answered 5,882 of
5,882 turns, none failing open; 1,424 of 1,432 cited evidence ids resolved to a turn. One run, 20 minutes,
four lanes; the ratings are cached, so a re-read is free and a repeat would re-ask the same temperature-0
model.

**What this does NOT say.** It prices affect as a predictor of FACT recall, on the field's benchmark. A
workload whose later relevance is itself emotional — a companion asking how the user felt about something —
is not measured, and nothing here has ground truth for query-less recall, which is the half of Ombre's
mechanism that bypasses a query. English only; one model rater. `ships=no`; **D169** records the refusal,
and both mechanisms remain expressible through seams that already exist.

### Scale: a default recall is WRITER-BOUND, and pure reads scale only without SQLite's allocation statistics (`memory-scale`, 2026-08-26 to 2026-09-08) <!-- result: id=scale-graph-sqlite-sequential arm="the graph engine on SQLite at shipped defaults, sequential — `node devtools/dev.mjs memory-scale` at 1k / 10k / 100k" metric=latency n="1k / 10k / 100k entries, hit-rate control 1.000 in every cell" value="recall p50 10.4 → 18.5 → 42.0 ms, p99 77 ms at 100k; writes 210–260/s at every size" ships=yes status=CURRENT -->

A COST measurement with no ground truth, so nothing here speaks to miss or pollution; `docs/memory.md` §7
carries what a deployment needs from it. It covers what `MemoryRecallBenchmarks` did not — that benchmark
runs the KEYWORD store, so the graph engine's own write and read paths were unmeasured at any size.

**Write throughput does not degrade** — 210–260 entries/s at every size, unchanged across 100× the store —
and **recall grows sub-linearly**, p50 `10.4ms → 18.5ms → 42.0ms` with p99 `77ms` at 100k. Storage is ~1 KB
per entry (100 MiB at 100k) and a cold first recall costs 21 → 49ms. Every cell reports a hit-rate control,
and it was `1.000` throughout — the latencies are real recalls, not fast misses (a recall matching nothing
is fast, and a table of fast empty recalls reads as good news). The `shipped` and `read-only` arms exist to
SPLIT a default recall's latency into the read and the write-back it performs afterwards, and the sweep runs
**sequentially** where every other sweep fans out, because contention cannot bias a rate and biases a
latency silently.

**Concurrency** — `memory-scale --concurrency`, 1k, five repeats per cell (2026-09-07): <!-- result: id=scale-concurrency-writer-bound arm="`shipped` and `read-only` at 1k, 1 to 8 workers — `memory-scale --concurrency`" metric=latency n="five repeats per cell" value="`shipped` p99 45.9 → 1061.6 ms at 8 workers, pinned near 160 recalls/s at every count" ships=yes status=CURRENT -->
...
a 5s `busy_timeout` under a 30s command timeout turns the lock into latency, so nothing reaches an error
log. The sweep reports CPU over wall time, and `shipped` keeps **0.2 cores** busy at every worker count —
threads genuinely blocked on the writer. `read-only` keeps **1.0 / 1.9 / 4.1 / 7.7**, so those threads run
flat out while throughput falls: one "contention here waits" reading covers the write-back arm and is wrong
about the read one. **Pure reads do not scale either, which WAL says they should** — `read-only` peaks at
TWO workers and falls to 368/s by eight, on a 22-core machine. Making `PRAGMA journal_mode=WAL` run once per
factory rather than per connection moved every cell inside its own spread (`shipped` 8-worker p99 1061.6 →
1115.1; `read-only` 2-worker rate 1024 → 1080), so that explanation is refuted and the experiment reverted.

**The read ceiling is SQLite's global memory-allocation STATISTICS, not a lock** (2026-09-08, **D107**). <!-- result: id=scale-sqlite-statistics-off arm="the same recalls with `SqliteRuntime.DisableMemoryStatistics` — the host's call, never set by Lyntai (D107)" metric=latency n="1 to 16 workers; three interleaved rounds at 8" value="340 → 4,665 recalls/s at 8 workers; 216 → 6,275 at 16" ships=no status=CURRENT -->
Maintaining them takes a process-global mutex on every allocation and free, and SQLite allocates heavily
inside an FTS5 query, so concurrent readers serialise on the counter. Turning them off changes nothing else:
...
**Every step to it was a refutation, and each is worth not repeating.** It is NOT the connection open (one
connection per worker, zero opens in the timed loop, same collapse), not GC (0% pause, allocation flat at
81 KB/op, Server GC changes nothing), not exceptions (zero first-chance), not the WAL (checkpointing 4 MB to
0 changes nothing), not journal mode, not the measurement window (a 10× window with a warmup reproduces it
at `hit` 1.000), not the engine (raw `SeedAsync` with no engine code collapses identically), and **not any
shared state** — one engine, one store and one database file per worker collapse exactly as the shared ones
do. What located it was that eight separate PROCESSES deliver 4,422 recalls/s where one process with eight
workers delivers 431: process-global, which a per-database isolation ladder can never reach.

**What a recall spends on LEARNING** — the write-back (reinforcement, co-activation edges, the review-log
row) as a share of the default recall's p50: **76% at 1k and 49% at 10k** at five repeats (2026-09-07), <!-- result: id=scale-writeback-share arm="`shipped` minus `read-only` p50 at 1k / 10k / 100k — the learning a default recall performs" metric=latency n="five repeats per cell" value="76% of p50 at 1k, 49% at 10k, 7% at 100k" ships=yes status=CURRENT -->
reproducing the 75% and 50% recorded before **D99** and **D101**. So the round-trip COUNT fell and the
latency share did not, which is consistent with D101: its claim is a count, and the write-back's cost is not
dominated by how many store calls it takes. A deployment that does not need learning turns it off
(`ReinforceOn = None`, `CoActivationCap = 0`, `LogReviews = false`) and recall roughly halves.
**At 100k the write-back is no longer the story**: 2.5ms of a 33.8ms recall, because the READ path is what
grows — `read-only` recall p95 runs ×11.02 from 1k→100k against `shipped`'s ×4.58 (write-back 4.2 → 4.0 →
2.5ms).

**Repeats are what made the share readable.** One repeat read 71% / 45% — an apparent 4–5 point
improvement that vanished under repeats — and 35% at 100k against **7%** at five, where the two arms' p50
spreads (32.9–36.3ms and 29.4–32.8ms) overlapped outright; the first run's 100k comparison came out
NEGATIVE, so the sweep printed "not readable" rather than an impossible percentage. Absolute latencies also
moved ~2.5× between a busy machine and a quiet one, so compare the growth factors, not the milliseconds.

**What this does NOT say.** Recall QUALITY at scale (no ground truth), Postgres, and any model in the loop —
an embedder, annotator or verifier would dominate every number here and none is wired.
`node devtools/dev.mjs memory-scale` reproduces it; `--concurrency` and `--repeat 5` the rest.
...
| records | first scan | scan p50 | scan max | in-memory match p50 | hits |
|---|---|---|---|---|---|
| 1,000 | 137.0 ms | 137.0 ms | 150.9 ms | 12.3 ms | 10 |
| 10,000 | 1,935.1 ms | **1,898.3 ms** | 1,935.1 ms | 69.1 ms | 100 |
| 100,000 | 17,708.1 ms | 18,900.9 ms | 19,324.9 ms | 783.7 ms | 1,000 |

**Reading the files costs 11–27× the match itself, and the bar fails even at 1,000 records.** So the package
holds each domain's records in memory and writes through (**D171**). The hits column is the
instrument check: one record in a hundred carries the query's phrase and no other record shares its words, so
exactly 1% must match — a first run whose filler vocabulary overlapped the query matched every record, and was
rerun before being read.

**What this does NOT say.** The cache was warm (the files had just been written) and this machine runs a
real-time antivirus scanner, so a cold cache or another machine differs in magnitude; nothing makes the scan
twenty times cheaper. One writer, no contention. `ships=no`: the scanning arm is refused.

### A recall writes back to the file graph store in two appends — 7.15–7.39 ms p50, under a third of ten rewrites (`storage-scan --writes`, 2026-09-24) <!-- result: id=storage-writes-recall-writeback arm="the shipped `WriteBackAsync` of the file graph store for one recall — 10 touches, 10 co-activated pairs, 10 review rows over 1,000 memories, two journal appends — against an atomic rewrite of a ~600 B record and a flushed 20-line append" metric=latency n="300 repeats per operation, two runs, one writer, the OS file cache warm, real-time antivirus on" value="7.15–7.39 ms p50 against a pre-registered bar of 21.53–22.02 ms, a third of ten rewrites at 6.46–6.61 ms p50; a flushed append 3.13–3.17 ms" ships=yes status=CURRENT -->

Not a memory-ENGINE figure: it decided the layout of the file-system backend's graph store (**D174**;
`docs/task-archive.md` Part 282). One recall writes back ten touches, twenty directed edges and ten review
rows, so the question was what that costs as files — a decay state in each recalled memory's header is ten
atomic rewrites, while a journal takes two appends. **Pre-registered**: the shipped write-back's p50 at most a
third of ten atomic rewrites' p50.

| operation (run 1 / run 2) | p50 | p90 | max |
|---|---|---|---|
| atomic rewrite of a ~600 B record | 6.46 / 6.61 ms | 8.07 / 8.47 ms | 10.73 / 328.58 ms |
| append 20 lines + flush to disk | 3.17 / 3.13 ms | 5.17 / 5.24 ms | 10.98 / 8.91 ms |
| shipped `WriteBackAsync` (10 touch, 10 pairs, 10 reviews) | **7.15 / 7.39 ms** | 9.30 / 9.65 ms | 28.76 / 38.21 ms |
| the bar: ten rewrites' p50 ÷ 3 | 21.53 / 22.02 ms | | |

**PASS on both runs: a recall's write-back costs little more than ONE atomic rewrite**, where the rejected
layout pays ten, so the journal ships (**D174**). The verdict reads p50 as pre-registered; run 2's 328.58 ms
