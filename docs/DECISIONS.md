...
**MEASURED 2026-09-09, and the trigger is INVERTED rather than uninformative.** The judge endorses the
SUPERSEDED fact 3.4× more often than the current one (36.2% against 10.8%, base rate 9.0%), so a penalty on
...
would still produce finite vectors. The synthetic fixture cannot see that; `Model2VecProviderLiveTests` ranks
...
*`Providers.*` holds the ADAPTER to something external, Core holds the LOGIC*: `Model2VecProvider` provides
...
**So the sections are symmetric and nullable.** `Chat = null` is an embeddings-only host; `Embeddings = null`
is a chat host; both set is the host that answers both. `Chat` defaults to non-null because a registration
...
**D131 read `Produces` being a list as "this host has two endpoints".** That is the wrong reading. A list
means one CALL can return several kinds — a multimodal model emitting text and an image. `/chat/completions`
and `/embeddings` are two services that share a hostname, and merging them made one provider report one id
for two backends, so a trace could no longer say which answered.

**The correction is that D127, D128 and D132 were fixing the OPPOSITE error, and the distinction is the
whole decision.** Those three took ONE backend that had been split across two interfaces and two `Add*`
methods and made it one. D131 took TWO backends and made them one object. Unifying the seam is right;
unifying the instances is not. "It is still a split" was the wrong diagnosis and produced the wrong fix.

**A chat model and an embedding model are different models, at different routes, with different wire
shapes.** What they share is a base URL and a key — transport, not identity. Repeating five lines of
transport config is the honest cost of that, and it is smaller than the cost D131 paid: a `Chat` section
beside an `Embeddings` section is a hardcoded taxonomy of exactly two kinds, sitting inside the object whose
premise is that `ProviderKinds` is an OPEN list. A reranking host would have grown a third section and a
rendering host a fourth, which is the same failure `Produces` being a string was chosen to avoid.
...
> **REVERSED by D137.** The suffix is back on every named backend. The reasoning below measured it against <!-- drift-ok: the amendment naming what it reverses -->
> the seventeen backends alone, where it is indeed redundant — but the builder has some sixty methods, and
> every other one names the KIND it registers. Against that population the suffix is the only thing telling
> `AddOllamaProvider` apart from `AddScorer`. What survives is the rule about the FACTORY primitives, which
> D137 keeps and generalizes.

`AddHttpProvider`, `AddOllamaProvider`, `AddOnnxProvider`, `AddClaudeCliProvider`, `AddModel2VecProvider`, `AddLlamaSharpProvider`, `AddFalProvider`,
`AddComfyUiProvider` — every backend registration drops the suffix.
...
`AddOpenAiProvider`, `AddOllamaProvider`, `AddOnnxProvider`, `AddClaudeCliProvider` — the suffix **D134**
removed is restored on all seventeen. It was never released without it.

**D134 compared the suffix against the wrong population.** It set the seventeen backend registrations beside
each other, found the suffix on all of them, and concluded it distinguished nothing. That is true and
irrelevant: the builder is not seventeen methods, it is about sixty, and **every other one names the KIND of
thing it registers** — `AddMemoryEngine`, `AddSecretVault`, `AddResponseCache`, `AddScorer`, `AddGuard`,
`AddJobHandler`, `AddRefusalMatcher`. Against THAT population the suffix is the whole signal.

**Dropping it made the backends the only group on the builder naming a vendor with no noun.** `AddOllama`
sitting between `AddMemoryEngine` and `AddRateLimit` gives a reader nothing: added what? A consumer scanning
for "how do I register a backend" had no shape to scan for, which is the opposite of the consistency D134
argued it was buying.

**The rule, stated so the next case is decidable:** a builder method is named for what it REGISTERS, and
the vendor is the qualifier. `AddOllamaProvider` reads as *a provider, the Ollama one* — the same shape as
...
**The cost of getting this wrong twice is the record, not the rename.** The methods were renamed, reverted,
...
it wrote. The graph engine indexes the vector BEFORE its similarity links and whether or not its neighbour
search succeeded, each step best-effort, so neither a failed link nor a failed search costs the vector. Storage
faults still throw; the storage tiers and the vector index are now REPORTED instead of silent, which is
`model-decoupling.md`'s "report which tier ran, on every result" applied to the write. An adopting
application's rebuild was recorded done while its embedder was down, and semantic recall stayed empty.

**Reading it is per engine kind** — each kind owes its own flag, stated once in
`docs/memory.md` §Know whether a write kept its vector. One enum serves both directions, so a write-only flag,
if one is added, is documented as never set on a recall.
