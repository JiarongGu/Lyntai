...
**Symptom.** None visible, which IS the defect. `AddOnnxCrossEncoderProvider` pointed at an NLI-shaped export —
...
it.** The asymmetry sat inside a single method — `OnnxCrossEncoderProvider` already ran one shape check at
...
other. `OnnxCrossEncoderProvider.FromDirectory` now resolves its output through `ScoreOutput`, against the graph's
