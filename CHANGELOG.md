...
- **A CLI spawn's tools can be chosen per call** (**D190**). `ICliToolProvisioner.ProvisionAsync(CliToolRequest, ct)`
  sees the request and which CLI is spawning (a default body keeps every existing provisioner working; a
  decorator must forward both members), and `McpToolHostOptions.ToolsByConsumer` maps a consumer to the tools its
  spawns host, resolved as `TimeoutByConsumer` is — the consumer's entry, then `"default"`, then every tool — so
  `ToolsByConsumer["default"] = []` denies by default. An empty list hosts none, and starts no host.
...
- **A remember reports what the write did: `IMemoryEngine.RememberAsync` returns a `MemoryWriteResult`**
  (**D175**) rather than a bare `MemoryRef`. `Reference` is the address it returned before; `Ran` names the
  tiers that took the write, the write side of `MemoryRecall.Ran`. A write stored WITHOUT its vector used to
  look exactly like one stored with it — the graph engine logged a failed embed and stored the entry anyway,
  and skipped the embed without a word while no embedder was available — so a rebuild run while the embedder
  was down was recorded as done and semantic recall stayed silently empty. **What to DO:** read `.Reference`
  wherever you used the returned `MemoryRef`; a caller that discards the result compiles unchanged but must be
  recompiled, since the return type changed; a BYO `IMemoryEngine` returns a `MemoryWriteResult` and reports
...
- **Live routing moves a ROUTE — a provider AND its model — rather than a model alone** (**D176**). The live key
  ...
  the first `:` so `ollama:qwen3:4b` is `ollama` plus `qwen3:4b`. An entry's model is its own, else the
  request's, else the backend's own default: a bare `provider` never takes the consumer's configured default
  (`DefaultModelByConsumer`, `LYNTAI_MODEL_<consumer>`), which was written for the candidates the route
  replaces. Text calls only — embeds, reranks and media under the same consumer do not move.
  ...
  naming no registered text provider, or a store that throws, logs a warning and the given candidates serve.
...
- **A live route outranks what the CALL named.** A route is a list of pairs, as configured candidates are, so it
  ...
  candidate's model does. The old model-only override ranked below both. A request's model that a route can
  never serve — every entry pins another, the case `AddMemoryVerification` and `AddMemoryAnnotation` already
  refuse at composition — logs a warning on each call. **What to DO:** where the request's own model must win
  for a routed consumer, write that route entry bare (`provider`, no model); where explicit candidates must win,
  keep that consumer's route key unset.
...
- **The tool loop asks the backend that will serve: one async `GetCapabilitiesAsync` replaces
  ...
  when a request carrying tools reaches a backend that does not declare tool calls. `GetCapabilitiesAsync` has
  no default body, so a BYO client or front-door decorator that does not answer it no longer compiles, rather
  than silently taking the prompt path. **What to DO:** a BYO `ITextClient` or `ITextRouter`, decorators
  included, implements `GetCapabilitiesAsync` — a decorator forwards it to the client it wraps (deriving from
  `DelegatingTextClient` inherits that), and a client that cannot say returns null — and deletes the old probes;
  a caller of them awaits `GetCapabilitiesAsync` and reads `SupportsToolCalls` / `SupportsStreamingToolCalls`
  off the answer, treating null as no native tool calls.

- **`LYNTAI_DEFAULT_CANDIDATES` reads through the one candidate-spec parser, so `provider:` means no model.** A
  trailing colon now reads exactly as a bare `provider` does, everywhere a candidate spec is read — here the
  request's model, else the configured default, else the backend's own. It used to yield an EMPTY model that
...
- **`UseDefaultCandidates(params string[])` reads each entry as a candidate spec**, as
  `UseDefaultMediaCandidates` and `LYNTAI_DEFAULT_CANDIDATES` already do: `"llama:qwen3"` is provider `llama`
  serving `qwen3`, where it used to be a provider NAMED `llama:qwen3`, which no backend matched. **What to DO:**
...
items are ordered by what each fix depends on — so "checklist 5" is an item number there, not one of the
guide's eight explanatory Steps. The reasoning for each change is `docs/DECISIONS.md` (**D45–D82**), and
what each working pass actually did is `docs/task-archive.md`.

> **This section records NET effect, not the order things landed.** 3.0 had a long development window in
> which several things were introduced and then changed again before release — a forgetting curve became the
> default and was later deleted, two constructor parameters were made plural and then renamed, a guard was
> added and then removed once its cause was fixed properly. A consumer upgrading from 2.5 meets only the end
> state, so that is what is written here. The step-by-step history is the commit log and the task archive.

### Breaking

**Memory — the retention model (D45–D62).** This is the bulk of 3.0 and the migration guide covers it in
order; the surface changes are:

- **Every policy seam takes one naming shape, `IMemory<Domain>Policy`** (**D47**). Two of the four reached a
  2.5 consumer and are real renames: `IMemoryClock` → `IMemoryAgePolicy` (there was never a clock — age is
  interference) and `IRetrievabilityPolicy` → `IMemoryRetrievabilityPolicy`. Implementations whose own names
  carried the retired word moved with them (`PerWriteClock` → `PerWriteAgePolicy`, and its three siblings).
  The other two seams are new in 3.0 and never shipped under any name. *Upgrade: checklist 1.*
- **The retention types moved into four domain namespaces** — `Lyntai.Memory.Interference` (age),
  `.Forgetting` (retrievability), `.Modulation` (retention), `.Salience`. No type's shape changed: the fix is
  an added `using`. `MemoryDecayState`, `MemorySignals` and the engine/storage contracts stay at the root.
  *Upgrade: checklist 1.*
- **`GraphMemoryEngine(memoryClock:)` is `agePolicies:` and takes a COLLECTION** — age and salience are
  plural seams now, because writes/characters/elapsed-time are different ASPECTS of "how much happened"
  rather than competing answers (**D48**). Each plural domain owns a composition policy; the engine composes
  nothing itself. Source-breaking only for a named-argument caller. *Upgrade: checklist 9.*
- **`IMemoryRetrievabilityPolicy.Reinforce` returns `MemoryDecayState`, not `double`**, so a richer model has
  somewhere to put what it owns — which `DsrRetrievability` immediately used for `Difficulty` (**D57**).
  Build the return with `with`, never from scratch. *Upgrade: checklist 3.*
- **Every retrievability and salience policy declares a `Provenance` bit**, so "never computed" is
  distinguishable from "computed as zero". Exactly one bit, never `None`; bits 32–62 are a consumer's own
  range and bit 63 is never set (both stores are signed 64-bit). *Upgrade: checklist 3.*
- **`IMemoryAgePolicy` gains `Age(MemoryAgeSample)` and a `Kind`.** Age is DERIVED from three primitives a
  store now tracks unconditionally rather than read from an accumulator, which is what makes a policy swap
  safe. `BurstDampenedAgePolicy` declares `Accumulating` because its position is path-dependent — it is the
  shipped default, so the default path is unchanged. *Upgrade: checklist 3.*
- **`IMemoryGraphStore` gains FIVE required members** — `DeleteAsync`, `RecordReviewsAsync`, `ReviewsAsync`,
  `RecordSubjectsAsync`, `NodesBySubjectAsync`. All are compile errors naming the member. `KnownSubjectsAsync`
  is the one addition with a default body. A store that does not want subject linking may no-op the subject
  pair; subjects steer LINKING and never recall. *Upgrade: checklist 4.*
- **Six records gained trailing members, so every positional `Deconstruct` of one needs more variables.**
  `MemoryDecayState` (`Signals`, `Difficulty`), `MemoryQuery` (`RankingPolicyName`), **`MemoryRecall`
  (`Answered`)**, and the store contract's `GraphNode` / `GraphNodeWrite` / `GraphTouch` / `GraphNeighbour`
  (signals, the three age primitives, provenance, difficulty). Additive in SOURCE for construction — every
  positional `new` still compiles — but BINARY-breaking, because C# bakes trailing-default call sites in at
  compile time.
  <br>**`MemoryRecall` is the one to check first**, because it is the only one on this list a consumer holds
  without implementing anything: it is what `RecallAsync` returns, and `var (items, ran) = recall;` no longer
  binds. `Answered` is 3.0's abstention signal — `true`/`false`/`null` for "a judge said something answered",
  "a judge said nothing did", and "nothing judged", which are three different facts.
- **`Stability` has ONE meaning, enforced by a contract fact**: the position delta at which retrievability is
  `0.5`. FSRS anchors at 90%; adopting that convention would silently reinterpret every stored value, so the
  fact exists to make it unshippable.
- **`MemoryRetentionPolicy` is `MemoryEvictionPolicy`** (and `LyntaiOptions.MemoryRetention` is <!-- drift-ok: the entry announcing a rename must name what was renamed -->
  `.MemoryEviction`, its `Eviction` property is `Mode`) — a rename resolving a collision 3.0 created. The
  seam `IMemoryRetentionPolicy` *lengthens* a half-life; this storage type *removes* entries, and in .NET
  `IFoo` reads as the interface of `Foo`. Same namespace, same presets, same behaviour, no schema change.
  *Upgrade: checklist 2.*
- **`IForgettableMemory` is SPLIT; `PruneAsync` moves to `IPrunableMemory`** (**D72**). The two verbs answer
  different questions — a forget must be COMPLETE, a prune is best-effort — and one interface forced an
  engine to claim both or neither, which a vector store cannot honestly do. Which members a removal visits is
  the new `IMemoryRemovalPolicy`. *Upgrade: checklist 13.*
- **`CompositeMemoryEngine` implements the removal capabilities, and `IForgettableMemory` gains
  `ForgetAsync`** (**D63**). Through 2.5.x the shipped memory subsystem had **no supported way to delete
  anything**: `Build` returns a composite for every registration, and the composite implemented neither.
  Removal fans OUT to every capable member and sums; a blend where none can remove throws rather than
  reporting `0`.

**Memory — changed defaults, which move behaviour without a compile error:**

- **`DsrRetrievability` is the only shipped forgetting curve** (**D49**). `HalfLifeRetrievability` and
  `HalfLifeOptions` are **DELETED with no restore path** — their central `× 1.5` reinforcement was admittedly
  unmeasured and compounded to 2.1× over a four-touch batch. **No data migration**: `Stability`'s unit
  contract is what made deleting a curve free. *Upgrade: checklist 5.*
- **`ReciprocalRankFusionPolicy` is the registered ranking default**, having beaten the multiplicative policy
  on the corpus's `topical` class in all six shapes (**D82** for why it ranks by competition).
  `MultiplicativeRankingPolicy` stays shipped and is one line to restore. *Upgrade: checklist 5.*
- **`DsrOptions.ReinforceGain` ships at `0` — a recall refreshes an entry's age but no longer lengthens its
  half-life** (**D54**). The single biggest behavioural change in 3.0, and the one that changes what an
  existing store returns without you touching anything. *Upgrade: checklist 5.*
- **An authoritative fact takes a slot WITHIN a recall's limit and displaces ordinary hits** (**D56**).
  Design §5.7.0 makes "never lose an authoritative fact" the only objective with no acceptable failure rate;
  the first measurement lost all three facts in all five languages. `GraphMemoryOptions.AuthoritativeReserve`
  bounds how many slots exact facts may take. **A small-limit recall returning fewer ordinary hits is the
  promise working.** No schema change — only the ordering was wrong. *Upgrade: checklist 6.*
- **`GraphMemoryOptions.HopAttenuation`, `.RelativeFloor` and `.SalienceRankWeight` moved to
  `MultiplicativeRankingOptions`** (`Lyntai.Memory.Ranking`), which validates on construction where
  `GraphMemoryOptions` accepted anything — a value 2.5 took in silence can now throw at startup.
  *Upgrade: checklist 7.*
- **`UseGraph` gains `retrievability`, `ranking` and `namedRankingPolicies`; `MemoryQuery` gains
  `RankingPolicyName`.** Source-compatible, binary-breaking. A curve and a ranking policy are both per-engine
  now (**D50**), and one call can select an alternate by name.

**Generation:**

- **`Lyntai.Generation` is no longer exempt from the SemVer promise** (**D70**). The package shipped <!-- drift-ok: the entry announcing the withdrawal must name what was withdrawn -->
  EXPERIMENTAL from 2.0.1 and the exemption is **withdrawn**, not merely satisfied. Nothing stops compiling;
  what changes is that these backends are now frozen, and a wire format differing in SHAPE rather than in a
  value is a major-version risk taken deliberately.
- **`IGenerationRouter` gains a third door, `StreamAsync`** (**D67**) — required, so a hand-written router
  must add it. Before this a backend advertising `GenerationDelivery.Stream` was unreachable through the
  platform. *Upgrade: checklist 14.*
- **`GenerationRouter` is a TRUST BOUNDARY: a backend that throws is classified and fallen over rather than
  propagating** (**D64**). Behavioural, with no compile-time signal — a caller catching an exception out of
...
- **`GenerationBackendsTool` takes an optional `GenerationOptions`** — source-compatible, binary-breaking.
  *Upgrade: checklist 15.*

**Jobs, CLI, and the naming sweep:**

- **`IJobStore` gains `TryAcquireSlotAsync`, `ReleaseSlotAsync` and `HeartbeatSlotsAsync`** for the
  cross-process job cap (**D73**), plus `PollAgainAsync`. A store that does not want the cap returns `null`
  from the acquire — a visible refusal rather than a silently ignored limit. *Upgrade: checklist 10.*
- **`ICliProviderDialect.BuildCompletionArgs` takes the tool-host args** and the engine no longer appends
  them (**D65**). Appending is correct for a CLI whose argv ends in options and wrong for one ending in a
  positional: on `codex` the MCP config landed in the prompt slot, so the tools were absent and **the turn
  was spent**. *Upgrade: checklist 13.*
- **The 3.0 naming sweep — names that MISLED are changed, names that merely differed are not** (**D66**).
  All source-level; seven retired spellings are fenced by `check-api-vocabulary`.
  ...
  | `IMemorySaliencePolicy.Appraise` / `IMemorySalienceCompositionPolicy.Compose` | `Signals` |
  ...
  | `IMemoryReapPolicy` / `MemoryReapKind` | `IMemoryRemovalPolicy` / `MemoryRemovalKind` |   <!-- drift-ok: a rename record NAMES the retired spelling -->

  <br>**The sharpest row:** `AuthoritativeReserve` named TWO quantities — recall SLOTS on
  `GraphMemoryOptions`, prompt CHARACTERS on `MemoryCompositionOptions` — in one namespace, with different
  null conventions, both reachable from a single builder chain. Reading "reserve 2" as slots set a
  two-character budget, truncating every authoritative fact to nothing. The slots one keeps its name.
  <br>**Deliberately NOT renamed** (**D66**): `LlmRequest req` vs `request` (74 signatures), `httpClient` on
  ten builder extensions, and `AgentStreamEvent`'s eight suffix-less subtypes. All real inconsistencies; none
  makes a reader believe a false thing, and a break must buy a reader something.
- **`MemoryEngineComposition` and `BudgetedGenerationRouter.RecordAsync` are now `internal`** — neither had
  an external caller.

### Added

- **Long-term memory can be told what a fact is ABOUT** — `AddMemoryAnnotation` has a model label each write
  with stable subjects, and entries about the same entity become connected. This is the fix for the measured
  no-graph floor: "my spouse is Alice", "she works at a hospital" and "we met in Kyoto" share no
  distinguishing word in any language, so no lexical or co-activation mechanism can link them.
- **`AddMemoryVerification`** — a model judges which of a recall's candidates actually ANSWERED the query
  (**D59**). It can RESCUE a low-ranked hit, records the outcome in the review log, and fails open: any
  failure yields "no opinion", never "nothing was relevant".
- **`JobOptions.GlobalMaxConcurrency`** — a job cap across every process sharing one store (**D73**). `0`
  (default) is the pre-3.0 unbounded behaviour with no extra round-trip.
- **The streaming contract carries native tool calls** (**D71**), so an agentic turn can finally stream —
  and a second capability says whether a given stream delivers them.
- **Streaming generation is reachable through the router** (**D67**), governed and throttled like the other
  two doors, with exactly one terminal chunk guaranteed.
- **Every unmeasured generation wire mapping is a host option** (**D69**) — fal's status vocabulary and cost
  fields, ComfyUI's response field names, and the whole `sd-cli` argv plus an `ExtraArgs` escape. A host that
  meets the real format edits configuration instead of waiting for a release.
- **`LocalDiffusionOptions.Accelerator` and `MaxDimension`** (**D68**) — the render size ceiling is DERIVED
  from a declared accelerator (`Cpu` derives a measured 768, `Gpu` derives none), never probed.
- **The review log** — one row per reinforcement, with a nullable `Verified`, so parameter fitting finally
  has something to read and the log can contain FAILURES rather than only successes.
- **Named LLM clients** — `AddLlmClient("memory-fast", c => c.UseProviders(…))`, resolved by name, so the
  memory subsystem's model calls can use a different backend from the chat path.
- **`NeutralSaliencePolicy`** — the supported way to turn salience OFF. Registering an empty collection does
  NOT do it: that means "take the shipped default", which is the one thing a consumer who wants nothing
  would try.
- **New memory knobs:** `SemanticSeedK`, `RecallReinforceCap`, `ReinforceOn`, `Reinforcement`,
  `ExpandCharBudget`, `AuthoritativeReserve`, `LogReviews`, `VerificationFilters`.
- **`JobOutcome.Poll`** — "not finished, and nothing went wrong; look again", which the contract had no word
  for.
- **`LlmRequest.Reasoning`** — ask a backend for no intermediate reasoning. Advisory; a backend that cannot
  express it ignores it. Measured stakes: ~25s versus ~1.5s per memory judgement.
...
- **Keyword recall matches TERM-WISE on every backend, and works in scripts that write no spaces** (**D55**).
  One tokenization for all three stores; a multi-word CJK query previously produced no search terms at all.
- **3.0 ships ONE memory migration, not six.** `M202608121100_MemoryRetentionModel` folds every unreleased
  memory change (**D9**); `M202608081215_MemoryGraph` is deliberately NOT folded in, because it shipped in
  2.5.0 and editing a migration a database has recorded by number is silently skipped. A fresh database
  applies **12** on SQLite and **13** on Postgres — the extra is a trigram index on `headline`, which SQLite
  does not need because its FTS5 mirror has covered it since the graph store shipped. The schema goldens,
  captured pre-squash, are the proof.

### Fixed

**Correctness a consumer can observe:**

- **A blended memory engine ignored `MemoryQuery.Limit` and `CharBudget`**, so an N-member blend returned up
  to N× what was asked for and could spend N× a prompt budget.
- **`ExpandAsync` accepted `hops` and `charBudget`, advertised `hops` to models, and ignored both.**
- **A curated engine's query-less recall returned the entire catalogue, unbounded.**
- **A failing embedder at recall emptied the whole recall** instead of degrading to the lexical hits.
- **A deleted memory left its SUBJECT rows behind on both SQL backends** — unbounded growth, and a recycled
  node id inherited the dead entry's links.
- **`GraphMemoryOptions.MinRetrievability` was read by nothing**, so `PruneAsync` with no explicit floor was
  a no-op.
- **A recall could return MORE entries than its own `Limit`.**
- **A durable render polled a backend that could never answer, forever** — fal reported every failure as
  "still running", so a dead render was never dead-lettered, never failed and never completed.
- **A CLI turn that finished cleanly could be reported as a stall, and paid for twice.**
- **A streamed turn that produced prose AND a tool call silently DROPPED the call** — no error, no verdict.
- **An OpenAI-compatible backend reporting a failure at HTTP 200 was never classified**, so the request was
  retried against a backend that had already refused it.
- **A headline-only query found the fact on SQLite and nowhere else**; authoritative facts are now genuinely
  admitted to a recall on every backend, and one the query never matched reports `Relevance` 0 everywhere
  rather than three different answers.
- **The in-process stores ranked by recency where their own interface docs promise matched-term count.**
- **Vector search now breaks a tie by id on every backend**, so top-k is repeatable.
- **A connected entry's decay was computed from two different age units at once**, and pruning papered over
  it by refusing to delete any connected entry — leaving a genuinely unretrievable one unreapable forever.
  All three age axes now speak one unit; the shipped default is byte-identical on every axis.
- **A non-finite value could reach persisted state** from three directions — an age tick, a `Difficulty`
  through the reinforcement path, and five `DsrOptions` that shipped unguarded. `MaxStability = NaN` was
  silent, permanent data corruption reachable from configuration.
- **A stability ceiling capped GROWTH but also CUT what was already stored** — a stored `100000` came back
  as `2000`, a 50× shortening, violating `Reinforce`'s own written guarantee.
- **A ranking score could reach `+Infinity` from finite inputs** in all three policies, and with
  `RelativeFloor = 0` that made the recall come back completely empty.
- **A salience below 1, or a non-finite one, behaved differently at every site that read it.**
- **`ElapsedAgePolicy` and `BurstDampenedAgePolicy` disagreed with themselves** when one instance served
  several named engines — one engine's writes aged another's memories.
...
- **Two MCP servers whose names differ only by `-` versus `_` sent one server's bearer token to the other's
  endpoint.**
- **The hosted MCP endpoint ran the application's tools with no guard gating at all**, and a thrown tool
  became an unrecoverable SDK error rather than the error observation `ITool` promises.
- **A tool-driven async render was never BILLED**, so a configured generation budget could not fire.
- **The memory subsystem's model calls are tagged `"memory"`** rather than billing to `"default"`.
- **`generate_backends` could block an agent for ~20 minutes**, and one throwing backend took the whole
  listing with it. Probes now run concurrently under `GenerationOptions.ProbeDeadline`.
- **ComfyUI and fal both reported a rejected credential as a failed RENDER**, which both benches the backend
  and tells a host nothing it can act on.
- **`PostgresMemoryGraphStore` opened every connection synchronously** — all fourteen sites.

**Documentation shipped to consumers:** two mangled sentences and three stranded `<summary>` blocks in
public XML docs, three class docs describing pre-3.0 relevance behaviour in the present tense, seven code
references to documents that no longer exist, and the migration guide's missing `KnownSubjectsAsync` note.

### Internal (no public surface change)

- **The relational stores share their materialization and, where the whole surface is dialect-free, their
  SQL** (**D77**, **D81**) — a column↔property mismatch is a silent null nothing can see, so two copies is
  two places for that silence to appear.
- **Every memory options record validates its domain through one guard** whose message is DERIVED from the
  check it makes (**D78**).
- **Comments have a written rule and a gate** — `.claude/rules/code-commentary.md` and `check-comments`.
  `src/` had reached 0.86 comment lines per line of real code, carrying 1.6× more prose than the decision
  log, the pitfalls record, the archive and the design contract combined.
- **Six new gates**: `check-counts`, `check-comments`, `check-links`, `check-samples`, `check-encoding` and
  `check-api-vocabulary`, plus tests for the guard scripts themselves — four of which had been failing
  PERMISSIVELY, printing a tick over an empty scan.
- **Contract coverage is now STRUCTURAL** across all seven memory policy seams, all five generation backends
  and every storage backend pair, rather than counted or asserted per-implementation.
