...
describe('check-counts — the counters, pinned against the real tree', () => {
  ...
  it('the option-guard counter counts CALL SITES, and matches the files D78 names', () => {
    ...
    // The claim is per-FILE as well as per-site, and D78's prose names ten files. A counter that only
    ...
    assert.equal(files.length, 10, `D78 says ten files; the tree has ${files.length}`);
