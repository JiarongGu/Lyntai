...
/** Ports this harness owns — `_llama-harness.mjs`' registry, which is what keeps every harness disjoint. */
export const PORTS = HARNESS_PORTS['memory-decision'];
 ...
 *
 *  <b>Parameterised by `ports`</b> because `tool-affordance` serves the same four roles on its own. The
 *  role table stays here and is IMPORTED rather than copied: the two sweeps' tables are read together, so
 *  a drifted weights list would make that comparison wrong rather than noisy. */
...
async function main() {
  ...
  await withOwnedServers({
    label: 'memory-decision', specs: serverSpecs(modelDir), scratchDir, serverExe, gpu: true,
    gpuNote: 'Accuracy is the metric here, so this bounds the WALL CLOCK rather than the result.',
    run: async () => {
