...
//
// THE COMMAND LIST THAT WAS HERE IS GONE, and its absence is the point. It named 30 of the 52 commands
// this file declares — the fourth hand-maintained copy of one list, and the third to go stale, after the
// usage string (24 of 30) and `CLAUDE.md`'s table (38 of 51) that D113 already made derived. A roster in a
// comment cannot be gated and this one had drifted before anybody looked.
//
// Where to look instead: `node devtools/dev.mjs` with no argument prints the list, read out of this file's
// own `case` labels · `CLAUDE.md` §Dev loop is the same list with a description, generated by
// `check-dev-loop --write` · `docs/GATES.md` says what each gate is FOR and which numbers it holds · and
// each command's ARGUMENT shapes and reasoning sit on its own `case` below, next to the code they govern.
...
switch (cmd) {
  ...
  // "what is the best memory configuration" read ~595 lines and got an unverifiable answer. Split
  // out as `docs/memory-measurements.md`, every result carries `<!-- result: … -->` and the index is
  // generated from those markers at ROW granularity — heading-level would be blind to a mid-section
  // retraction. `SUPERSEDED` is never authored: it is derived from a later row's `supersedes=`, because
  // two-sided bookkeeping is where this rots (D114).
  case 'check-measurements': {
    run('node', [path.join(repo, 'devtools', 'scripts', 'check-measurements.mjs'), ...args]);
    break;
  }
