...
/// the claim predicate (stale lease). All transitions/fencing execute the SHARED <see cref="JobStoreSql"/>
/// statements (booleans bound as parameters), so the state machine can't drift from the SQLite store; only
/// the claim SQL is dialect-specific. <c>timestamptz</c> compares as real timestamps.
/// </summary>
...
{
    // shared columns, qualified for the UPDATE … FROM … RETURNING (id is otherwise ambiguous with `pick`)
    private static readonly string JCols =
        string.Join(", ", JobStoreSql.Cols.Split(", ").Select(c => "j." + c));
    ...
    public async Task<JobRecord?> ClaimNextAsync(string lane, string workerId, TimeSpan lease, CancellationToken ct = default)
    {
        ...
        await using var conn = await factory.OpenAsync(ct).ConfigureAwait(false);
        // candidate predicate is the SHARED dialect-neutral text; only this FOR UPDATE SKIP LOCKED
        // locking frame is Postgres-specific
    ...
    /// <inheritdoc />
    /// <remarks>The same two statements SQLite runs, differing only in the LOCKING frame — and here
    /// <c>FOR UPDATE SKIP LOCKED</c> works FOR the cap rather than against it. On the JOB table it is why a
    /// count cannot gate a claim; on a SLOT table two workers skipping to two different rows is exactly the
    /// intended outcome, so the cap is exact AND claiming stays parallel.</remarks>
    public async Task<int?> TryAcquireSlotAsync(int cap, string workerId, TimeSpan lease, CancellationToken ct = default)
    {
        ...
        var taken = await conn.QuerySingleOrDefaultAsync<int?>(new CommandDefinition($"""
            ...
            FROM (
                ...
                FOR UPDATE SKIP LOCKED LIMIT 1) pick
            WHERE t.slot_index = pick.slot_index
            RETURNING t.slot_index
    ...
    {
        // Guarded here, not by the dialect — IJobStore.ListAsync. An unguarded negative LIMIT throws here.
        if (limit <= 0) return [];
        await using var conn = await factory.OpenAsync(ct).ConfigureAwait(false);
