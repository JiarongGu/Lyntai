...
/// <summary>DI entry points for the OpenAI <c>codex</c> CLI: the completion provider and the self-driving
/// agent session. A consumer composes them through the builder and never constructs their types by
/// hand.</summary>
public static class CodexCliBuilderExtensions
{
    /// <summary>Register the OpenAI `codex` CLI provider (id "codex-cli" unless <paramref name="id"/> names
    /// another). With no arguments the spawned command honors <c>LYNTAI_PROVIDER_CMD</c> / <c>CODEX_CMD</c>
    /// env overrides (tests/e2e point these at a deterministic stub), then falls back to <c>codex</c> on PATH.
    /// If an <see cref="ICliToolProvisioner"/> is registered for this provider id, the CLI is given the app's
    /// registered tools over MCP.</summary>
    ...
    /// is preferred, then the one keyed on "codex-cli", then an unkeyed one.</param>
        ...
        string id = CodexCliProvider.ProviderId)
    {
        ...
        builder.AddProvider(sp => new CodexCliProvider(
            ...
            command,
            CliComposition.Provisioner(sp, id, CodexCliProvider.ProviderId),
            environment,
            backend,
            id));
        return builder;
    ...
    /// Registered both unkeyed and KEYED by <paramref name="id"/>, so an app offering both CLI backends resolves
    ...
    /// <remarks>Read <see cref="CodexAgentSession"/>'s remarks before adopting: a tool step's KIND is reached
    /// by elimination and is provisional, while its PAYLOAD is codex's own; and two neutral
    /// <see cref="AgentSessionOptions"/> members (disallowed tools, system prompt) are handled explicitly
    /// because codex has no equivalent.</remarks>
    /// <param name="builder">The Lyntai builder.</param>
