...
{
    /// <summary>Resolve the configured dialect, turning the <see cref="HttpDialect.Auto"/> sentinel into a
    /// concrete dialect via URL detection. Every consumer of a dialect should start here.</summary>
    internal static HttpDialect ResolveDialect(HttpDialect configured, string? baseUrl) =>
        configured == HttpDialect.Auto ? ProviderDetect.Detect(baseUrl) : configured;
    ...
    /// code path.</summary>
    internal static void ApplyAuth(HttpRequestMessage request, string? apiKey, HttpDialect dialect)
    {
        if (string.IsNullOrEmpty(apiKey)) return;
        request.Headers.Authorization = new("Bearer", apiKey);
        if (dialect == HttpDialect.AzureOpenAi)
