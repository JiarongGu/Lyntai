...
public sealed class SqliteKeyValueStore(IDbConnectionFactory factory) : IKeyValueStore
{
    public async Task<string?> GetAsync(string key, CancellationToken ct = default)
    {
        ...
        return await conn.QuerySingleOrDefaultAsync<string>(
            new CommandDefinition("SELECT value FROM app_config WHERE key = @key", new { key }, cancellationToken: ct))
    ...
    public async Task DeleteAsync(string key, CancellationToken ct = default)
    {
        using var conn = factory.Open();
        await conn.ExecuteAsync(new CommandDefinition(
            "DELETE FROM app_config WHERE key = @key", new { key }, cancellationToken: ct)).ConfigureAwait(false);
    }
}
