...
/// caching and throttling live on the <see cref="ITextClient"/> front door, ABOVE the router, so a router
...
public interface ITextRouterFactory
{
    ...
    ITextRouter For(IReadOnlyList<ProviderRegistration<IModelProvider>> providers);
    ...
    ITextRouter For(IReadOnlyList<IModelProvider> providers);
}

/// <inheritdoc cref="ITextRouterFactory"/>
...
public sealed class TextRouterFactory(
    IProviderPool<IModelProvider> pool,
    DeadHostTracker deadHosts,
    LyntaiOptions options,
    ILoggerFactory? loggers = null,
    IModelRoutingStore? modelRouting = null,
    IProviderAdmission? admission = null) : ITextRouterFactory
{
    /// <inheritdoc/>
    public ITextRouter For(IReadOnlyList<ProviderRegistration<IModelProvider>> providers)
    {
        ...
        return new TextRouter(instances, deadHosts, options, loggers?.CreateLogger<TextRouter>(), modelRouting,
    ...
    /// <inheritdoc/>
    public ITextRouter For(IReadOnlyList<IModelProvider> providers)
    {
        ArgumentNullException.ThrowIfNull(providers);
        return new TextRouter(providers, deadHosts, options, loggers?.CreateLogger<TextRouter>(), modelRouting);
    }
}
