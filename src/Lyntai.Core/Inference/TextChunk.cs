...
public enum TextChunkKind
{
    ...
    /// <summary>The stream ended in an error; <see cref="TextChunk.Verdict"/> says how.</summary>
    Error,

    /// <summary>The model asked for a tool. <see cref="TextChunk.ToolCall"/> carries it, COMPLETE — a
...
public sealed record TextChunk
{
    ...
    /// <summary>Meaningful on <see cref="TextChunkKind.Error"/> (and Ok on Final).</summary>
    ...
    public static TextChunk Tool(TextToolCall call) => new() { Kind = TextChunkKind.ToolCall, ToolCall = call };

    public static TextChunk Final(TextUsage? usage = null, string? detail = null) =>
        new() { Kind = TextChunkKind.Final, Usage = usage, Detail = detail };

    public static TextChunk Error(ProviderVerdict verdict, string? detail = null) =>
        new() { Kind = TextChunkKind.Error, Verdict = verdict, Detail = detail };
}
