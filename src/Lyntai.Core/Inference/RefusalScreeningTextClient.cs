...
/// token).
/// </summary>
public sealed class RefusalScreeningTextClient(
    ITextClient inner,
    IEnumerable<IRefusalMatcher>? matchers = null,
    ILogger<RefusalScreeningTextClient>? logger = null) : DelegatingTextClient(inner)
{
    private static readonly TimeSpan MatchTimeout = TimeSpan.FromMilliseconds(100);
    private readonly IReadOnlyList<IRefusalMatcher> _matchers = [.. matchers ?? []];
    private readonly ILogger _logger = logger ?? NullLogger<RefusalScreeningTextClient>.Instance;
