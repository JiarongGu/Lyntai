...
public interface ITextClientFactory
{
    ...
    /// <param name="name">The client's name, as passed to <c>AddTextClient</c>.</param>
    ...
    ITextClient Get(string name);

    /// <summary>The default client — the container's own <see cref="ITextClient"/>, the one every existing
    ...
    bool TryGet(string name, out ITextClient client);

    /// <summary>Every registered name. Does not include the default, which has none.</summary>
    IReadOnlyList<string> Names { get; }
}

/// <summary>The default <see cref="ITextClientFactory"/> over the registered named compositions.</summary>
...
public sealed class TextClientFactory(
    IReadOnlyDictionary<string, ITextClient> named,
    ITextClient fallback) : ITextClientFactory
{
    /// <inheritdoc />
    public IReadOnlyList<string> Names { get; } = [.. named.Keys.OrderBy(n => n, StringComparer.Ordinal)];

    /// <inheritdoc />
    public ITextClient Get(string name)
    {
        ...
        return named.TryGetValue(name, out var client)
            ...
            : throw new KeyNotFoundException(
                Names.Count == 0
                    ? $"No LLM client is named '{name}'. None is registered — call AddTextClient(name, …)."
                    : $"No LLM client is named '{name}'. Registered: {string.Join(", ", Names)}.");
    }

    /// <inheritdoc />
    public ITextClient Get() => fallback;
