...
/// <summary>Configures ONE named LLM client inside <c>AddTextClient(name, c => …)</c> — the counterpart of
/// the memory engine's own <c>AddMemoryEngine(name, e => …)</c> callback.</summary>
public sealed class TextClientBuilder
{
    internal TextClientBuilder(string name) => Name = name;
    ...
    ///
    /// <para><b>Ids, not instances</b>, matching how <see cref="ITextRouter"/> already selects candidates and
    ...
    public TextClientBuilder UseProviders(params string[] providerIds)
    {
    ...
    public TextClientBuilder UseCandidates(params ProviderCandidate[] candidates)
    {
...
public static class TextClientRegistration
{
    ...
    /// <see cref="ITextClientFactory"/>.</para>
    /// </summary>
    ...
    /// engine.</exception>
    public static LyntaiBuilder AddTextClient(this LyntaiBuilder builder, string name,
        Action<TextClientBuilder>? configure = null)
    {
        ArgumentNullException.ThrowIfNull(builder);
        if (string.IsNullOrWhiteSpace(name))
            throw new ArgumentException("An LLM client needs a name.", nameof(name));

        var client = new TextClientBuilder(name);
        configure?.Invoke(client);

        if (!builder.NamedTextClients.TryAdd(name, client))
