...
internal static class ProviderPoolGuard
{
    ...
    /// rejects a pair an ordinal comparison would wave through. Never make a router's lookup ordinal to match
    /// a slot's case: <see cref="EnsureIdMatchesSlot"/> accepts a slot cased differently from the provider's
    /// <see cref="IProviderIdentity.Id"/>, and an ordinal lookup would leave that backend poolable and never
    /// tried.</para></summary>
