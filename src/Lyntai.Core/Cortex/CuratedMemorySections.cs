...
public static class CuratedMemorySections
{
    ...
    public static string Compose(IEnumerable<CuratedMemory> entries,
        ...
        string? taskKey = null, IEnumerable<string>? scopes = null)
    {
        header ??= k => $"## {k}";
        ...
        if (taskKey is not null)
        {
            ...
            enabled = enabled.Where(e => AppliesTo(e, taskKey, scopeSet));
        }
        var byKind = enabled
    ...
    /// and <see cref="Compose"/>): the entry's <see cref="CuratedMemory.TaskKey"/> is null (applies to every taskKey)
    /// or equals <paramref name="taskKey"/>, AND either <paramref name="scopes"/> is empty (scope filter disabled),
    ...
    public static bool AppliesTo(CuratedMemory entry, string taskKey, IReadOnlyCollection<string> scopes)
        => (entry.TaskKey is null || entry.TaskKey == taskKey)
