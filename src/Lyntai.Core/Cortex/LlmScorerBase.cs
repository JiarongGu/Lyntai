...
public abstract class LlmScorerBase(ITextClient llm) : IScorer
{
    ...
    /// <summary>Routes <see cref="IScorer.Applies"/> to the protected override above. A PROTECTED member cannot
    /// implicitly implement an interface member, so without this <see cref="IScorer"/>'s always-true default
    /// would answer for every judge. Explicit, so it adds nothing to the public surface.</summary>
