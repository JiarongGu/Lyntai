...
/// <summary>
/// A non-<see cref="ProviderVerdict.Ok"/> outcome raised as an EXCEPTION — the counterpart of
...
/// <see cref="ProviderVerdict.NotConfigured"/> ("never set up", so the host should offer SETUP rather than
...
/// </summary>
public sealed class LlmVerdictException(ProviderVerdict verdict, string? detail = null)
    : InvalidOperationException($"lyntai: {verdict} — {detail}")
{
    ...
    /// <see cref="ProviderVerdict.NotConfigured"/>, which is not a fault at all (<c>docs/DECISIONS.md</c> D31) — and
    /// <see cref="ProviderVerdictExtensions.IsTransient(ProviderVerdict)"/> answers "is a retry worth one attempt?".</summary>
    public ProviderVerdict Verdict { get; } = verdict;
