...
/// <para>Collapsing is not truncation — no characters are dropped, so a verbatim promise still holds. What
/// a multi-line fact does lose is its line breaks, which is the price of the one-line invariant.</para>
/// <para><see cref="string.ReplaceLineEndings(string)"/> rather than a <c>\n</c> replace: it also folds
/// <c>\r</c>, <c>\f</c>, <c>\v</c> and the Unicode separators U+0085, U+2028 and U+2029, any of which
/// starts a line for some reader downstream.</para></summary>
internal static class MemoryLine
{
    public static string Flatten(string content) => content.ReplaceLineEndings(" ").Trim();
}
