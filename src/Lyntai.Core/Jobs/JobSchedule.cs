...
/// OR a <see cref="Cron"/> expression — exactly one. Registered with
/// <c>builder.AddJobSchedule</c>/<c>AddCronSchedule</c>, which refuse a malformed or duplicate schedule; the
/// app drives <see cref="IJobScheduler"/> (host-free). The next-run time is persisted keyed by
/// <see cref="Name"/>, so it must be STABLE + UNIQUE across schedules.</summary>
