...
description: Project facts live in the repository, versioned and reviewable; global memory is user preferences only.
...
memory is for facts about the *person* — role, communication style, personal preferences — that hold across
every project they touch. Everything about *this* project goes in the repository.
...
| An always-on convention every task needs | `.claude/rules/` |
| A deep dive read only when touching that area | `.claude/knowledge/` |
| Anything else about this project | route it by kind — `persist-working-state.md` §Route by KIND |
...
carve-outs saying never to compress a destructive-action warning, and never to abbreviate an identifier or
a commit message — is doctrine, however much it looks like a preference. Losing it on a fresh clone loses
the carve-outs, and the carve-outs are the part that was learned the hard way.
...
- **Learned something durable about this project? Write it to the repository**, in the row above that fits.
...
- A product's own memory feature is a different thing entirely: this rule is about the assistant's memory,
  never the library's own `IMemoryStore` / `ICuratedMemoryStore` / `ISemanticMemory` / `IMemoryEngine`.
