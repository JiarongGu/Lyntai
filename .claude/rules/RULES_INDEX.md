...
| [dev-conventions.md](dev-conventions.md) | Any code change | Package layout (interface in Core, impl in adapter, never adapter→adapter), naming (never `Dto` — name the type for what it IS; use `*Options`/`*Request`/`*Row`/…), async Dapper + FluentMigrator numbering + FTS5-trigram, spawn hygiene + fallback semantics, DI-collection variation points, xUnit + provider-stub testing, devtools loop |
