...
| [repo-mechanics](repo-mechanics.md) | applying a general rule in this repository — package names, version authorship, the dev loop, guard scripts, local model servers, or scratch paths | contract in Lyntai.Core with adapters, never adapter-to-adapter; zero Dto identifiers; never hand-edit VersionPrefix or the Unreleased heading, and never NAME a version that has not shipped; everything before 3.0 is HISTORY — log it, never analyse it; llama.cpp is the standard local model server; scratch under `devtools/_*`; line endings are LF, declared in `.gitattributes` |
...
| [windows-machine](windows-machine.md) | running any shell command, script, or file write on a Windows development machine | never round-trip text through PowerShell 5; BOM and encoding traps; ask the `.gitattributes` attribute before `core.autocrlf`; exit codes that lie; never kill a shared runtime by name |
...
| [model-decoupling](../knowledge/model-decoupling.md) | designing or building any feature that uses — or could use — a language model, an embedding model, or any AI service, including one you are about to specify with only model-free options | specify the feature without naming a model; select the provider by deployment; report which tier ran; list a model-assisted option among the candidates and price it on the corpus at hand |
