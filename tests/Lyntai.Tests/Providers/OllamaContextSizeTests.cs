...
/// <summary>The context-window override on <see cref="HttpModelOptions"/> is a NATIVE-Ollama wire
...
/// generic name a consumer on the OpenAI dialect reasonably set it and it did nothing, silently.
...
public class OllamaContextSizeTests
{
    ...
    private static HttpModelProvider Provider(StubHttpHandler handler, Action<HttpModelOptions> configure)
    {
        var config = new HttpModelOptions { BaseUrl = "https://api.openai.com", ApiKey = "test-key" };
        configure(config);
        return new HttpModelProvider("openai-compat", config, () => new HttpClient(handler, disposeHandler: false),
