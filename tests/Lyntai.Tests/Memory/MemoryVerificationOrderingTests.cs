...
/// <summary>The one fixture <see cref="MemoryVerificationOrderingTests"/> and <see cref="MemoryVerdictFusionTests"/>
/// share: four entries one query matches, and a judge that endorses by headline.
...
/// the baseline warmed.</para></summary>
internal static class VerdictOrderingFixture
{
    ...
    /// </summary>
    internal sealed class Endorses(string headline) : IMemoryVerificationPolicy
    {
    ...
    internal static async Task<IReadOnlyList<string>> RecallAsync(
    ...
    internal static async Task<IReadOnlyList<string>> BaselineAsync()
    {
...
public class MemoryVerificationOrderingTests
{
    private static Task<IReadOnlyList<string>> RecallAsync(TempDb db, IMemoryVerificationPolicy? verifier,
        int limit) => VerdictOrderingFixture.RecallAsync(db, verifier, limit);
    ...
    public async Task An_endorsed_candidate_leads_the_page_with_filtering_off()
    {
        ...
        using var db = new TempDb();
        var judged = await RecallAsync(db, Endorses(baseline[2]), limit: 10);
    ...
    public async Task An_endorsed_candidate_below_the_limit_is_rescued_onto_the_page()
    {
        ...
        using var db = new TempDb();
        var judged = await RecallAsync(db, Endorses(baseline[3]), limit: 2);
    ...
    public async Task A_judge_that_endorses_what_already_leads_returns_an_identical_page()
    {
        ...
        using var db = new TempDb();
        var judged = await RecallAsync(db, Endorses(baseline[0]), limit: 10);
