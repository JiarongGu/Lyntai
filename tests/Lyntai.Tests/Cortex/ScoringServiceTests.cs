...
public class ScoringServiceTests
{
    ...
    // caller's cancel. A bare `catch (OperationCanceledException) { throw; }` would therefore let one scorer's
    // own deadline sink the whole evaluation — discarding every score already computed, since `results` is a
    // method-local — against a type doc promising "skips null/faulted results".
    ...
    {
        // The sharper half: order matters, because a rethrow would throw away the earlier result too.
    ...
    {
        // The control: cancellation belongs to the caller and must NOT be swallowed. Without this, "skip
        // every OperationCanceledException" would pass everything above.
    ...
    {
        // Same shape one layer down: its own log line promises "results still returned".
