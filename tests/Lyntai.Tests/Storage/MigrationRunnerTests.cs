...
public class MigrationRunnerTests : IDisposable
{
    ...
    public void Fresh_db_has_all_migrations_applied()
    {
        ...
        Assert.Equal([202607170001, 202607170002, 202607170003, 202607170004, 202607170005, 202607170006, 202607170007, 202607170008, 202607180001, 202607180002], versions);
    }
    ...
    public void Migrating_again_is_idempotent()
    {
        ...
        using var conn = _db.Factory.Open();
        Assert.Equal(10L, conn.ExecuteScalar<long>("SELECT COUNT(*) FROM lyntai_version_info"));
    }
}
