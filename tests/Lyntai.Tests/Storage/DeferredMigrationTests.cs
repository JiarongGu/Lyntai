...
{
    private readonly TempDbPath _db = new("deferred"); // fresh un-migrated path — deferral is the point
    ...
    public async Task First_store_access_migrates_and_round_trips()
    {
        ...
        Assert.True(File.Exists(_db.Path));
    ...
    public async Task Migration_runs_exactly_once_under_concurrent_first_access()
    {
        ...
        Assert.All(opens, count => Assert.Equal(13L, count)); // all migrations applied, once
    }
}
