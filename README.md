...
- **Memory can learn what a fact is ABOUT** (opt-in). `AddMemoryAnnotation()` labels each written fact with
  ...
  unconditionally, and `AddMemorySubjectSeeds` is only there to CONFIGURE it — once an annotator is wired: a
  handle exists only because you paid for it, so reading it back needs no second opt-in.
...
await memory.RememberAsync(new MemoryWrite("proj", "code",
    "The build gate is node devtools/dev.mjs verify, which runs 24 checks."));
...
| `ComfyUiProvider` | **Job** | *Measured against a live server, image, video and mesh workflows.* Local and workflow-driven: you supply the graph in `Options["workflow"]` (+ optional `Options["prompt-path"]` to place the prompt), and outputs come back as view URIs — a mesh as `model/gltf-binary`. Each input (bytes, or a URI it fetches from any http(s) server, capped by `MaxFetchBytes` and never with ComfyUI's credentials off its own origin, unless a client you supply follows redirects itself — validate URIs a model supplies) is uploaded and its stored name written at the field `Options["input-path"]` names, or `Options["input-path:<role>"]` for an input with a role; an input with nowhere to go is refused, never dropped. A transport failure while polling reports **Running, not Failed** — an unanswered status call says nothing about a run still going — while a 4xx or an unconfigured base URL stays terminal, so a bad id never polls forever |
...
```
node devtools/dev.mjs verify           # THE "am I done?" gate — 24 checks, stopping at the first
